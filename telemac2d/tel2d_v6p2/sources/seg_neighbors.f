...
!
      ...
      IF(ERR.NE.0) THEN
        IF(LNG.EQ.1) WRITE(LU,*)'SEG_NEIGHBOR: MAUVAISE ALLOC JVOIS'
        IF(LNG.EQ.2) WRITE(LU,*)'SEG_NEIGHBOR: WRONG ALLOC OF JVOIS'
        STOP
      ENDIF
!
      EPS=1.D-5
!
!  NEX  : POINTER FOR THE FLOOWING NODE IN AN ELEMENT
!
! 1. INITIALIZATION.
!
      DO IS = 1 , NPOIN
...
!        ITS NORM
         NORMNUB =DSQRT(XNUB12**2 + YNUB12**2)
...
!              NORM OF MNUBO1 AND MNUBO2
               NORM_MNUBO1 = DSQRT((XM-XX(NUBO1))**2 + 
     &                             (YM-YY(NUBO1))**2)
               NORM_MNUBO2 = DSQRT((XM-XX(NUBO2))**2 + 
     &                             (YM-YY(NUBO2))**2)
...
!                               2- WHERE M IS AT THE RIGHT OF NUB2 (N1--N2--M)
               IF((NORM_MNUBO1+NORM_MNUBO2).GE.NORMNUB
     &       .AND. NORM_MNUBO2.GE.NORM_MNUBO1) THEN
!                 NORM OF JM
                  NORM_JM= DSQRT((XX(J)-XM)**2 + (YY(J)-YM)**2) 
!                 NORM OF JNUBO1
                  NORM_JNUBO1 = DSQRT((XX(J)-XX(NUBO1))**2 + 
     &                                (YY(J)-YY(NUBO1))**2) 
!                 SIN OF THETA
...
!                 NORM OF JM
                  NORM_JM= DSQRT((XX(J)-XM)**2 + (YY(J)-YM)**2) 
!                 NORM OF JNUBO2
                  NORM_JNUBO2 = DSQRT((XX(J)-XX(NUBO2))**2 + 
     &                                (YY(J)-YY(NUBO2))**2) 
!                 SIN OF THETA
