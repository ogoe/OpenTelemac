...
C                                                                       
      EPS =  1.D-6   
C IF NOT CFL VARIABLE WE IMPOSE CFL=0.5
C      IF(.NOT.DTVARI)CFLWTD=0.5D0                                    
C                                                                       
...
111   CONTINUE
C
C CALCUL DU DT QUI SATISFAIT CFL
c      CALL VFCFL(NUBO,VNOIN,NSEG,NPOIN,X,Y,G,H,EPS,QU,QV,DT,CFLWTD,
c     *           DTVARI,LISTIN)  
...
C                                                                       
      ...
      ENDDO
C
C  TIME STEP UNDER CFL CONDITION
C 
C CALCUL DU DT QUI SATISFAIT CFL
      CALL CALDT(NPOIN,G,HN,U,V,DTHAUT,DT,CFLWTD,ICIN,DTVARI,LISTIN)
C
...
C
      IF(LT.EQ.1) THEN
C
C     INITIALISATIONS AT 1ST TIME STEP
C     ********************************
C
       WRITE(LU,*) ' '
       WRITE(LU,*) '          *********************** '
