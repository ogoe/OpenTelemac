...
!
  ...
  !
  ! case of date and time in selaphin file
  !
  if(iparam(10)==1) then
        read(fu_s,iostat=f_error) (ivdate(i),i=1,6)
        if(f_error/=0) then
      write(*,*) 'Error reading date and time!'
      error=f_error
      goto 110
        end if
  endif
...
!
  if(binout) then
      ...
      write(fu_v,'(A6,I7,1X,I8)',iostat=f_error)'CELLS ',&
                                nelements,               &
