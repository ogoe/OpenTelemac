...
!=======================================================================
!
! PUT THE MESH INTO TELEMAC FORMAT
!=======================================================================
!
      WRITE(LU,*) "CALL VERIFI"
      EPSI = 1.D-6
      CALL VERIFI(MESHINIT%X%R,MESHINIT%Y%R,IKLE,NCOLOR,TRAV1,EPSI,
     &            3,3,MESHINIT%NPOIN,MESHINIT%NELEM,
     &            MESHINIT%NELMAX)
!
...
!
     ...
     &                CHBORD,TBOR,ATBOR,BTBOR)
        ZF%DIM1= MESHINIT%NPOIN
        H%DIM1 = MESHINIT%NPOIN
        ! FIXME: SHOULD BE GENERIC
        DO I=1,MESHINIT%NPOIN
          ZF%R(I) = 0.D0
        ENDDO
        WRITE(LU,*) "IN STBTEL REFINE MESH", MESHINIT%NPOIN
        DO ITRAC=1,NTRAC
          TEXP%ADR(ITRAC)%P%DIM1=MESHINIT%NPOIN
          TTILD%ADR(ITRAC)%P%DIM1=MESHINIT%NPOIN
          TN%ADR(ITRAC)%P%DIM1=MESHINIT%NPOIN
        ENDDO
      ENDIF
!
...
!
      IF(OPTASS) THEN
        WRITE(LU,*) "CALL RENUM"
        CALL RENUM
     &  (MESHINIT%X%R,MESHINIT%Y%R,WORK,IKLE,NBOR,TRAV1,TRAV2,TRAV3,
     &   NCOLOR,COLOR,MESHINIT%NPTFR)
      ENDIF
!
