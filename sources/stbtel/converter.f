...
!
      IF(LNG.EQ.1) THEN
        ...
        IF(DEBUG) WRITE(LU,*) 'FICHIER D ENTREE        : ',LOC_INPFILE
     ...
     &                         LOC_BNDFILE
        ...
        IF(DEBUG) WRITE(LU,*) 'FICHIER DE SORTIE   : ',LOC_OUTFILE
        IF(DEBUG) WRITE(LU,*) 'FORMAT DU FICHIER DE SORTIE : ',OUTFMT
        IF(DEBUG) WRITE(LU,*) 
     &      'FICHIER DES CONDITIONS LIMITES EN SORTIE : ',LOC_OUTBNDFILE
        IF(DEBUG) WRITE(LU,*) 'FICHIER LOG EN SORTIE  : ',LOC_OUTLOGFILE
      ENDIF
      IF(LNG.EQ.2) THEN
        IF(DEBUG) WRITE(LU,*) '---FILES INFORMATION---'
        IF(DEBUG) WRITE(LU,*) 'INPUT FILE        : ',LOC_INPFILE
        IF(DEBUG) WRITE(LU,*) 'INPUT FORMAT      : ',INFMT
        IF(DEBUG) WRITE(LU,*) 'BOUNDARY FILE     : ',LOC_BNDFILE
        IF(DEBUG) WRITE(LU,*) 'LOG FILE          : ',LOGFILE
        IF(DEBUG) WRITE(LU,*) 'OUTPUT FILE       : ',LOC_OUTFILE
        IF(DEBUG) WRITE(LU,*) 'OUTPUT FORMAT     : ',OUTFMT
        IF(DEBUG) WRITE(LU,*) 'OUT BOUNDARY FILE : ',LOC_OUTBNDFILE
        IF(DEBUG) WRITE(LU,*) 'OUT LOG FILE      : ',LOC_OUTLOGFILE
      ENDIF
!
...
!      
      IF(INFMT .EQ. 'SERAFIN') THEN
        CALL READ_SERAFIN(LOC_INPFILE,LOC_BNDFILE)
...
!
      IF(OUTFMT .EQ. 'SERAFIN') THEN
        CALL WRITE_SERAFIN(LOC_OUTFILE,LOC_OUTBNDFILE)
