...
!
     ...
     &               NGEO,NFO1,IPOBO,IPARAM,DATE,TIME)
        IF(NOMBND2(1:1) .NE. ' ') THEN
          CALL LECSELLIM (NBND2,LIHBOR,LIUBOR,LIVBOR,HBOR,UBOR,VBOR,
...
!
     ...
     &              TRAV1,TRAV2,SHP,ELT,NPMAX)
        CALL FILL_LIM
     & (NPTFR,NPMAX,0,LIHBOR,LIUBOR,LIVBOR,DUMMY,
     &  HBOR,UBOR,VBOR,CHBORD,DUMMY,DUMMY,DUMMY)
      ELSE
...
!
      ...
      IF(DIV4 .AND. NOMBND2(1:1).NE.' ') THEN
        CALL WRITESELLIM
     &(NLIM,LIHBOR,LIUBOR,LIVBOR,HBOR,UBOR(:,1),VBOR(:,1),
     & CHBORD,NBOR,NPMAX,NPTFR)
      ELSE
