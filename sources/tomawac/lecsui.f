...
!       SETS TRIPLETS U,V,TV1 AND 2 TO UV,VV,AT
      ...
      ELSE
        CALL FIND_VARIABLE(FFORMAT,NPRE,VAR_NAME(I),TRA01,NPOIN,
     &                 ISTAT,RECORD=-1)
        CALL CHECK_CALL(ISTAT,'LECSUI:FIND_VARIABLE')
      ENDIF
!
...
!
      IF(COURAN.OR.VENT) THEN
        IF(VENT) THEN
          CALL FIND_VARIABLE(FFORMAT,NPRE,VAR_NAME(I),TRA01,4*NPOIN2,
     &                   ISTAT,RECORD=-1)
          CALL CHECK_CALL(ISTAT,'LECSUI:FIND_VARIABLE')
        ELSE
          CALL FIND_VARIABLE(FFORMAT,NPRE,VAR_NAME(I),TRA01,2*NPOIN2,
     &                   ISTAT,RECORD=-1)
          CALL CHECK_CALL(ISTAT,'LECSUI:FIND_VARIABLE')
        ENDIF
      ENDIF
