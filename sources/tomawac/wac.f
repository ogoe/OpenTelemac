...
!-----------------------------------------------------------------------
!
!     QINDI_WAC ALLOCATED ONLY AT FIRST CALL
!
      IF(.NOT.DEJA_WAC) THEN
        ALLOCATE(QINDI_WAC(NPLAN))
        DEJA_WAC=.TRUE.
      ENDIF
!
!=====C
...
!
     ...
     &  STSDER%R,STSDER%R,STSDER%R,STSDER%R,NPTFR,'WAC',.FALSE.,
     &  WAC_FILES(WACGEO)%FMT,WAC_FILES(WACGEO)%LU,
...
!        EX-SCX      EX-SCY (MEMORY OPTIMISATION)
     ...
     &   IELM3    , DIFFRA   , MAREE.OR.PART.EQ.0 ,ISUB)
!Fin COUPLAGE
...
!        EX-SCX      EX-SCY (MEMORY OPTIMISATION)
     ...
     &   SPHE     , PROINF    , PROMIN,MESH,MESH3D,MESH%IKLE,TB,
     &   IELM3    , DIFFRA    , MAREE.OR.PART.EQ.1 ,ISUB )
!Fin COUPLAGE
...
!
     ...
     &  WAC_FILES(WACVEB)%NAME,WAC_FILES(WACVEB)%FMT,
     &  WAC_FILES(WACVEF)%NAME,WAC_FILES(WACVEF)%FMT,NBD,QINDI_WAC,
     ...
     &  NVWIN,DIAGHF,VEGETATION,SDSCU,CDSCUR,CBAJ)
...
!
     ...
     &               WAC_FILES(WACRBI)%FMT,STSDER%R,MESH3D)
