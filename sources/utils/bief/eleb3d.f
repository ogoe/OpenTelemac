...
!
      ...
      INTEGER, INTENT(INOUT) :: IKLBOR(NELEBX,4)
...
!
      DO IETAGE = 1,NETAGE
        DO IPTFR = 1,NPTFR
          ...
          IKLBOR(IELEB,1) =        IPTFR  + (IETAGE-1)*NPTFR
          IKLBOR(IELEB,2) = KP1BOR(IPTFR) + (IETAGE-1)*NPTFR
          IKLBOR(IELEB,3) = IKLBOR(IELEB,2) + NPTFR
          IKLBOR(IELEB,4) = IKLBOR(IELEB,1) + NPTFR
          IPOIN = NBOR(IPTFR)
          NBOR(IPTFR +(IETAGE-1)*NPTFR)=IPOIN+(IETAGE-1)*NPOIN2
          IELEM = NELBOR(IPTFR)
          IF(IELEM.GT.0) THEN
            NELBOR(IELEB)=IELEM+(IETAGE-1)*NELEM2
