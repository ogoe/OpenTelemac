...
!                                        HERE IKLE2
      ...
      DOUBLE PRECISION, INTENT(INOUT) :: XCONV(NG),YCONV(NG),ZCONV(NG)
      DOUBLE PRECISION, INTENT(INOUT) :: SHP(3,NG)
      DOUBLE PRECISION, INTENT(INOUT) :: SHZ(NG)
!
!+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
!
...
!
      IF(NG.NE.NELEM*NGAUSS.AND.IELM.EQ.11) THEN
        IF(LNG.EQ.1) THEN
          WRITE(LU,*) 'CHAR_GAUSS : MAUVAIS NOMBRE DE POINTS'
        ENDIF
        IF(LNG.EQ.2) THEN
...
!
      ELSE
!
        IF(LNG.EQ.1) WRITE(LU,10) NGAUSS,IELM
        IF(LNG.EQ.2) WRITE(LU,11) NGAUSS,IELM
10      FORMAT(1X,'CHAR_GAUSS : OPTION NON PREVUE :'    ,I6,
     &            ' AVEC IELM=',I6)
