...
!       REAL (KIND=R8), POINTER, DIMENSION (:)  :: ptr2            
                                         
      !  assumed-shape arrays:  Uebergabe von Datenfeldern an Unterprogramme
      !> ohne Groessenangaben(engl. assumed-shape arrays). Dies funktioniert
      !> nur wenn in der aufrufenden Programmeinheit der interface-Block fuer
      !> das Unterprogramm angefuehrt wird.
      INTERFACE !-----------------------------------------------------+                  
...
!    &      , AVAIL, KNOLG, KNOGL, dzCL_sis                )          !
     ...
     &      , AVAIL, KNOLG, dzCL_sis                       )          !
          ...
          REAL (KIND=R8),INTENT(IN)    :: dts             ! time-step-duration  [ s ]
          REAL (KIND=R8),INTENT(IN)    :: time            ! time [ s ]
          REAL (KIND=R8),INTENT(IN)    :: ELAY0           ! activLayerThickness  [ m ]
          REAL (KIND=R8),INTENT(IN)    :: z_sis (:)       ! current bottom [ m+NN ] assumed-shape array
         !REAL (KIND=R8),INTENT(IN)    :: x_sis (:)       ! x-koord [ m ] assumed-shape array
         !REAL (KIND=R8),INTENT(IN)    :: y_sis (:)       ! y-koord [ m ] assumed-shape array
