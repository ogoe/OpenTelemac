...
!***                                              ********************************************
     ...
     &   , ip, P1km, P2km, angleABCD  )             
      ...
      REAL (KIND=8),INTENT(IN)::  xA,yA,zA, xB,yB,zB     ! points:  A,B  ( profile1 )
     &                           ,xC,yC,zC, xD,yD,zD     !    "     C,D  ( profile2 )
      REAL (KIND=8),INTENT(IN)::  P1km, P2km             ! measure of river length: profile1, profile2  
      REAL (KIND=8),INTENT(IN)::  angleABCD              ! angle between profile1 and profile2  
      INTEGER,INTENT(IN)      ::  ip                     ! index of profile1
        
      !------- local variables ---------------
      INTEGER         :: i   !  , status
        
      REAL (KIND=8)   :: xAB,yAB , xCD,yCD                    ! vetkors: AB, CD
      REAL (KIND=8)   :: skaSASN                              ! skalar_produkt( SA, SN )  
      REAL (KIND=8)   :: xS,yS,  xN,yN                        ! points:  intersection S, field node N
      REAL (KIND=8)   :: xSN,ySN                              ! vetkor: SN (between intersection S and field node N)
      REAL (KIND=8)   :: xSA,ySA,  xSC,ySC                    ! vetkors: SA, SC
      REAL (KIND=8)   :: rL1,rR1,  rL2,rR2,   rN              ! distances (radii): L1,R1,L2,R2,N to S
      REAL (KIND=8)   :: zR1,zL1,  zL2,zR2                    ! z values ( L1,R1,... see sketch below)  
      REAL (KIND=8)   :: DetSASN, DetSCSN                     ! determinants of Vektors:  SA SN  and  SC SN
      REAL (KIND=8)   :: z1, z2                               ! interpolated z-values between zR1,zL1 and zL2,zR2       
      REAL (KIND=8)   :: phi                                  ! angle between SA and SN
      REAL (KIND=8)   :: rLphi, rRphi                         ! angle, radii as function of phi
      REAL (KIND=8)   :: intpolFaktRadial, intpolFaktAngle    ! interpolation factors
        
      REAL (KIND=8), DIMENSION(4) ::    DistanceToS
      REAL (KIND=8)               :: maxDistanceToS, minDistanceToS
      !REAL (KIND=8) :: r2d = 565.48667764602_8 ! = 360/2*Pi conversion factor: radian to degree ! debug only
      ...
      WRITE(6,*)'?>-------  SR Intpol_Z_angular_Profils -----'
     ...
     &  )  Call ErrMsgAndStop2(
     &  " ",1,"reason: Profiles must not intersect or touch !",46
     & ," ",1,"        Fit profile  ",21, ip,  SRname, ipid       )
      ...
      DO i = 1 , F%nNodes !____________________________________________|
        ...
        IF(     (DetSASN < 0.0_8 .AND. DetSCSN < 0.0_8)
     &      .OR.(DetSASN > 0.0_8 .AND. DetSCSN > 0.0_8)  )  CYCLE    ! node is not between !WRITE(6,'("Node not between lines ",2I3,g15.6)') ip, ip+1         ! debug
     ...
     &      .OR.(  rN  < MIN(rLphi, rRphi) )     )  CYCLE ! node is not between radii   !WRITE(6,'("Node not inside max or min radii ",2I3,g15.6)')      ! debug
        ...
        IF( .NOT. (phi >= 0.0_8 .AND. phi <= angleABCD) )  CYCLE  ! node is not between angle !WRITE(6,'("Node not between the angle ",2I3,g15.6)')      ! debug
      ...
      WRITE(6,*)'?>-------  SR Intpol_Z_angular_Profils END--'
      RETURN                             
!***                                              ********************************************
!***                                              ********************************************
