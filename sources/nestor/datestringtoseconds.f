...
!SR  & ( timeString, timeSec )           
     & ( timeString, lineCount )         
      ...
      REAL (KIND = 8) :: DateStringToSeconds    !> seconds since a "time-zero-point" (function name)
                                         
      INTEGER        :: stat             
      INTEGER        :: diffDays ,diffMinutes ,diffHours ,diffSeconds
      INTEGER(kind=8):: diffDays8,diffMinutes8,diffHours8,diffSeconds8
...
!663   FORMAT(' ?>',2(/,' ?>'))            ! 3 lines like "?>         "
     ...
     &  ,"         reason: format conflict, date-time format",50
     &  ,"                 must be:   yyyy/mm/dd-hh:mm:ss   ",50 
     ...
     &                + diffMinutes8  * 60_8       !  the "_8" tells the compiler     
     &                + diffHours8    * 3600_8     !> that he has to deal with a  
     &                + diffDays8     * 86400_8    !> integer kind=8 constant
      ...
      WRITE(6,*)'?>-------  SR DateStringToSeconds End ------'
