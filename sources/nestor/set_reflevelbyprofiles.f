...
!***                                              ********************************************
     & ( F, fileNameRefLev )                 
      ...
      REAL (KIND=8)   :: xA,yA,zA,xB,yB,zB,xC,yC,zC,xD,yD,zD  ! points:  A,B,C,D 
      REAL (KIND=8)   :: xAB,yAB , xCD,yCD                    ! vetkors: AB, CD
      REAL (KIND=8)   :: absAB, absCD                         ! length of vetkors AB,CD
      REAL (KIND=8)   :: skaABCD, skaCDCD                     ! skalar produkts   
      REAL (KIND=8)   :: r, angleABCD                         ! relation, angle(AB,CD), angle(SA,SN)
      ...
      WRITE(6,*)'?>-------  SR Set_RefLevelByProfiles -------'
      ...
      IF(linecount <= 0) Call ErrMsgAndStop2(
     ...
     &  P(ip)%x1,P(ip)%y1,P(ip)%z1,P(ip)%x2,P(ip)%y2,P(ip)%z2,P(ip)%km ! debug  
      ...
      DO ip=1, nProfiles - 1 !__________________________________________|
        ...
        !  check for angle bigger or equal  90°                        |
        !  calculate the angle ________________________________________|
        IF( r <= 1.e-5_8  )Call ErrMsgAndStop2("      ", 6
     ...
     &    ,"occured at profile: ",20, ip, SRname, ipid      )
        ...
        IF( angleABCD < 1.e-5_8  ) THEN         ! profiles are almost parallel
     ...
     &          , ip, P(ip)%km, P(ip+1)%km,  angleABCD     )   
      ...
      IF( ANY(F%refZ < -1000.0_8) ) Call ErrMsgAndStop2("     ", 5
     ...
     &," ",1, -1, SRname, ipid      )
      ...
      WRITE(6,*)'?>-------  SR Set_RefLevelByProfiles End ---'
      RETURN                             
!***                                              ********************************************
!***                                              ********************************************
