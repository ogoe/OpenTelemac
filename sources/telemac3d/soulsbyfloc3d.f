...
!
!***********************************************************************
! TELEMAC3D   V6P1                                   29/11/2011
!***********************************************************************
!
!brief    COMPUTES THE FALL VELOCITY of mud flocs based on R. Soulsby's latest
!        , 2011, formulation taken from a method based on Manning's floc database.
!
!history
!+        29/11/11
!+        V6P1
...
!
!
! COMPUTE FALL VELOCITY SO THAT IT IS A FUNCTION OF SUSPENDED CONC (AT PREVIOUS TIME STEP)
! USING THE FORMULA DERIVED BY RICHARD SOULSBY (2011) AT HRW. 
!
!
      DENS    = 2.64D0 ! RELATIVE WATER SEDIMENT DENSITY (HARD CODED FOR NOW AFTER SOULSBY [2011])

!     OPTIMISE VARIABLES WHERE POSSIBLE (OUTSIDE LOOP)
      NU_WCUB = NU_W*NU_W*NU_W ! KINEMATIC VISCOCITY OF WATER CUBED
      DI4     = 1E-20       ! di^4
      DM4     = 1E-16       !(d_micro)^4
      EXP1    = LITK*2.672D0

!      II      = 0 ! INITIALISE 2D COUNTER
...
!      IF (II>NPOIN2) II=1          
! CV      
      USTAR  = SQRT(UETCAR(IPOIN)) ! Direct from TMAC - NB do not use since not damped, use TOB
!     USTAR  = MAX((SQRT(TOB(IPOIN)/RHO_FIX)),1E-25) !EQ. (32) Soulsby (1997)
      ...
      IF (HN%R(IPOIN).LT.0.01) THEN
         WCHU%R(IP) = 0.0002D0
      ELSE
          
!     COMPUTE DIMENSIONLESS CONCENTRATION IN SAME WAY AS SOULSBY (I.E USING WATER DENSITY)
!     NOTE THAT SEDIMENT MUST BE THE LAST TRACER (I.E. THE NTRAC ADDRESS IN THE BIEF OBJECT TA)

!     NOTE ALSO THAT UNITS OF TRACERS ARE WHATEVER ARE INPUTTED AS INITIAL CONDITIONS I.E.
...
!     NB: AS UNITS OF RHO_FIX ARE KG/M3 IT IS IMPORTANT THAT THE SED CONC IS IN KG/M3
...
! (we have used min depth of 0.01 above to prevent problems with zero depth)
      EPS    = (USTAR*USTAR*USTAR)*ETA/(KAPPA*Z)
...
!     GET WEIGHTED AVERAGE (BASED ON RR) TO PROVIDE THE EFFECTIVE SETTLING VELOCITY:
      ...
      ! DEBUGGING
      !if (IP.eq.2211) then
          !write(*,*) 'USTAR=',USTAR,' ETA=',ETA,' EPS=',EPS,'WCHU=',WCHU%R(IP)
      !endif
