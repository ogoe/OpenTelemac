      PROGRAM API_DUMMY_EXE_T2D
        ...
        INTEGER COMM,NCSIZE2,ipid2,color
        INTEGER NPLAN
...
!
        color = ipid/2
        CALL MPI_COMM_SPLIT(MPI_COMM_WORLD,color,IPID,COMM,IERR)
        print*,'mpi_comm_split',ierr
        CALL MPI_COMM_SIZE(COMM,NCSIZE2,IERR)
        print *,ierr
        CALL MPI_COMM_RANK(COMM,IPID2,IERR)
        print *,ierr
...
!
        CALL RUN_SET_CONFIG_T2D(ID,666+COLOR,LNG,COMM,IERR)
        PRINT *, 'IERR',IERR
        PRINT *, 'ID',ID
