...
!+        08/02/2016
!+        V7P1
!+   Secutity added to avoid ALTBUS value to be lower than bottom
!
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!
      INTEGER N
!
      DOUBLE PRECISION XSOR,YSOR,ZSOR,XENT,YENT,ZENT
...
!
        ...
        IF(NCSIZE.GT.1) THEN
          ...
          ZENT=P_DMAX(ZENT)+P_DMIN(ZENT)
          XSOR=P_DMAX(XSOR)+P_DMIN(XSOR)
          YSOR=P_DMAX(YSOR)+P_DMIN(YSOR)
          ZSOR=P_DMAX(ZSOR)+P_DMIN(ZSOR)
        ENDIF
        ALTBUS(N,1) = MAX(ALTBUS(N,1),ZENT)
        ALTBUS(N,2) = MAX(ALTBUS(N,2),ZSOR)
        DX  = XSOR-XENT
        DY  = YSOR-YENT
        ANG = DATAN(DY/DX)
        ANGBUS(N,1) = ANG
