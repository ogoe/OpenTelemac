...
!
              IF(S1.LT.(RD1+HAUT1).AND.S1.LT.(RD2+HAUT1)) THEN
...
!               PRESSURE FLOW --> ORIFICE LAW
                SECBUS(N) = LARG * HAUT1
                IF(S1.GE.(RD1+HAUT1)) THEN
                  Q = SECBUS(N)*SQRT(2.D0*GRAV*(S1-S2)/(CE1+L))
                ELSE
...
!               PRESSURE FLOW --> ORIFICE LAW
                SECBUS(N) = LARG * HAUT1
                IF(S2.GE.(RD2+HAUT1)) THEN
                  Q = - SECBUS(N)*SQRT(2.D0*GRAV*(S2-S1)/(CE2+L))
                ELSE
...
!
     ...
     &     DBUS(N) = 0.D0
!
! THIS WOULD GENERATE TOO MUCH TEXT IN THE OUTPUT FILE WHEN USING A LOT OF CULVERTS
