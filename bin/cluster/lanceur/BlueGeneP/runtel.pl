...
#	   dans le repertoire de travail apres contrÃÂÃÂle de leur existence.
...
#                  : On utilise les chemins complets des fichiers de paramÃÂÃÂtres (REPLANCE)
#
...
#                     - option de suppression ou non du rÃÂ©pertoire temporaire aprÃÂ¨s calcul
#                     - compilation/link prÃÂ©alable ÃÂ  l'acquisition des fichiers et
#                       option de compilation/link seul "-cl"
#                     - amÃÂ©lioration de la dÃÂ©tection et gestion des erreurs
#                     - mot clef LK_LIB_SPECIAL pour des librairies spÃÂ©cifiques
...
#Localiser les modules dans le rÃÂ©pertoire 'bin'
...
#             En cas d'erreur, une entree de de type est retournÃÂ©e :
#              ("H_ERROR","Error msg")
#--------------------
#
{
...
#------------
    ...
    if ($VALIDATION eq "T") {         # CORRECTION ($VALIDA)   $DCSA$ ÃÂÃÂ  verif
	print "\n";
...
#        ("LISTE DES FICHIERS"/"LIST OF FILES" les ÃÂÃÂ©numÃÂÃÂ¨re)
#  - FH        : handle du fichier
#  - %motsdic  : mots clefs ÃÂ  considÃÂ©rer (reference)
...
#Cas d'un fichier de type FORTRAN : le repÃÂÃÂ©rer pour compilation avec son nom
...
#Cas d'un fichier de type FORTINC : le repÃÂÃÂ©rer pour l'acquÃÂÃÂ©rir en mÃÂÃÂªme
...
#Cas d'un fichier de type CONLIM : le repÃÂÃÂ©rer pour PARTEL
    if (@valsd[0]=~/;CONLIM/) 
...
#Cas d'un fichier de type SELAFIN-GEOM : le repÃÂÃÂ©rer pour GRETEL
...
#Ecriture des donnÃÂÃÂ©es du fichier pour son acquisition/restitution par 'runcode'   
...
#  - versdef  
#-------------
#Definition : Retourne les caractÃÂÃÂ©ristiques d'un code 
{
#Initialisations/recup arguments
  my $refH  = @_[0];
  my $NOMCOD= @_[1];
...
sub get_mots
#-------------
#--- Acquisition des mots clÃÂÃÂ© ÃÂÃÂ  partir des fichiers
#    cas et dico
#E:
#  - NOMCOD  : nom du code
...
if (scalar(@vals) == 0)
  {  ecrire("ERREUR : Mot clef du dictionnaire 'NUMERO DE VERSION' non defini",
            "ERROR : undefined dictionary parameter 'NUMERO DE VERSION'");
     exit 1;}
$vs=@vals[0];          #traitement spÃÂÃÂ©cifique pour accepter la forme
...
printf F "      } \n";
# RBR 09-07/2004 Modif pour gÃÂÃÂ©stion UNIX et WIN
#RBR printf F "  } \n";
