...
#Verif noms des parametres
    if (  not (    exists($hashref->{'Functions'})
       ...
       { $hashref->{'H_ERROR'} = "Invalid licence parameters for $GENERIQUE";
         return(0);
       }
...
#Function value
    if ( ($hashref->{'Functions'} & 16384 ) != 16384 )
       { $hashref->{'H_ERROR'} = "Invalid Function parameter value for $GENERIQUE licence";
         return(0);  }
#Ptype value
    if ( ($hashref->{'Ptype'} & 16384 ) != 16384 )
       { $hashref->{'H_ERROR'} = "Invalid Ptype parameter value for $GENERIQUE licence";
         return(0);  }
#StartPeriod value
    if (     (  substr($hashref->{'StartPeriod'}, 4,1) ne "/" )
          || (  substr($hashref->{'StartPeriod'}, 7,1) ne "/" )
          || (  length($hashref->{'StartPeriod'})  != 10      )   )
       { $hashref->{'H_ERROR'} = "Invalid StartPeriod parameter value for $GENERIQUE licence";
         return(0);  }
#EndPeriod value
    if (     (  substr($hashref->{'EndPeriod'}, 4,1) ne "/" )
          || (  substr($hashref->{'EndPeriod'}, 7,1) ne "/" )
          || (  length($hashref->{'EndPeriod'})  != 10      )   )
       { $hashref->{'H_ERROR'} = "Invalid EndPeriod parameter value for $GENERIQUE licence";
         return(0);  }
#HostId value
    if ( length($hashref->{'Hostid'})  < 4      )
       { $hashref->{'H_ERROR'} = "Invalid Hostid parameter value for $GENERIQUE licence";
         return(0);  }
#Password value
    if ( length($hashref->{'Password'})  != 16      )
       { $hashref->{'H_ERROR'} = "Invalid Password parameter value for $GENERIQUE licence";
         return(0);  }

#Transmission au code des parametres de protection via les
#variables d'environnement
      $ENV{"STELP1"}=join "_", uc($GENERIQUE), $hashref->{'Functions'}, $hashref->{'Ptype'};
...
#-------------
{
  ...
  printf "\n";
  printf "=======================================================================\n";
  printf " Telemac System V5P4 to V5P5   Perl scripts dated:  12th November 2003 \n";
  printf " Modified and checked by jaj for BAW      Tue Dec 23 09:37:52 MET 2003 \n";
  printf "=======================================================================\n";
  printf "$_[0]\n";
  printf "\n";
}
...
#Remplissage du Hash de definition des parametres de protection
     %hprot=();
     %hprot=fillHashFromBloc($sysprotIni,uc($GENERIQUE),%hprot);
     if ( defined $hprot{"H_ERROR"} )                        #Erreur : bloc pas trouvé
            { print "ERROR : No licence information for $GENERIQUE :\n";
              print "        $hprot{\"H_ERROR\"}\n";
              exit; }
     if ( scalar(keys %hprot) < 6 )                          #Erreur : pas 6 elements
            { print "ERROR : Invalid licence parameters for $GENERIQUE\n";
...
#=================================================================================
#   TENTATIVE DE MODIFICATION POUR COUPLAGE ESTEL3D/TELEMAC2D
#
#=================================================================================
...
#=================================================================================
#   FIN DE TENTATIVE DE MODIFICATION POUR COUPLAGE ESTEL3D/TELEMAC2D
#
#=================================================================================
...
$dfilename=join "", $REPLANCE,$ps,"delete_",$PARA,$WORKING,".bat";
$sfilename=join "", $REPLANCE,$ps,"signal_",$PARA,$WORKING,".bat";
...
if ($LNG == 2 )
   { printf F "print \"Working directory '$REP'  cleaned.\\n\"; \n"; }
else
   { printf F "print \"Repertoire de calcul '$REP' detruit.\\n\"; \n"; }
printf F "}\n";
