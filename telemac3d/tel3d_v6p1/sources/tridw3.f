...
!
!***********************************************************************
! TELEMAC 3D VERSION 6.1   04/12/09  J-M HERVOUET  (LNHE) 01 30 87 80 18
...
! 
!
!      LIKE TRIDW2, BUT WITH A CORRECTION OF BOTH HORIZONTAL
!      AND VERTICAL FLUXES TO GET A DIVERGENCE-FREE VELOCITY FIELD.
!                                                             
!      COMPUTATION OF AN AVERAGED VALUE OF H * WSTAR IN A WAY
!      COMPATIBLE TO THE PSI SCHEME
!      SEE PHD OF J-M HERVOUET, EQUATION 5.58
!
...
!
!
!      25/10/2004 : DONE WITH MASS-LUMPING, 
!                   SEE ALSO MT14PP FOR COMPATIBILITY.
!
!
!-----------------------------------------------------------------------
!                             ARGUMENTS
...
! !  DT            ! -->! PAS DE TEMPS                                 !
! !  SM            !<-->! SECOND MEMBRE POUR LA VITESSE VERTICALE      !
! !  TRAV2         !<-->! STRUCTURE DE TABLEAUX DE TRAVAIL 2D          !
! !  TBB           ! -->! BLOC DE BLOCS DE TRAVAIL                     !
! !  W1            !<-->! TABLEAU DE TRAVAIL (CALCUL DES MATRICES...)  !
! !  AM1,AM2       !<-->! MATRICES DE TRAVAIL                          !
! !  WSBORF,L,S    ! -->! CONDITIONS AUX LIMITES DIRICHLET POUR WS     !
! !  LIWBOF,L,S    ! -->! TYPE DE CONDITIONS LIMITES POUR WS           !
! !  NBOR          ! -->! ADRESSES GLOBALES DES POINTS FRONTIERES.     !
! !  NPLAN         ! -->! NOMBRE DE PLANS DU MAILLAGE 3D               !
! !  NETAGE        ! -->! NPLAN - 1                                    !
! !  NPOIN2        ! -->! NOMBRE DE POINTS 2D                          !
! !  NELEM2        ! -->! NOMBRE D'ELEMENTS 2D                         !
! !  NPTFR         ! -->! NOMBRE DE POINTS FRONTIERE DU MAILLAGE 2D    !
! ! KENT,....,KLOG ! -->! TYPES DES CONDITIONS LIMITES PHYSIQUES       !
! !  EPSW          ! -->! PRECISION POUR LE CALCUL DE WS               !
! !  NITW          ! -->! NOMBRE D'ITERATIONS POUR LE CALCUL DE WS     !
! !  PREW          ! -->! PRECONDITIONNEMENT POUR LE CALCUL DE WS      !
! !  SOLW          ! -->! SOLVEUR POUR LE CALCUL DE WS                 !
...
! !  IELM2         ! -->! TYPE DE DISCRETISATION 2DH                   !
...
!-----------------------------------------------------------------------
!
! SOUS-PROGRAMME APPELE PAR : PRECON
! SOUS-PROGRAMMES APPELES : MATRIX , SE2ELM , OS , MTRXVC , SOLVE
!
!***********************************************************************
!
      USE BIEF
...
!     FLUINT PART OF THE PRESSURE EQUATION RIGHT HAND SIDE
     ...
     *            MESH3D,MSK,MASKEL)
C     ON RETURN MESH3D%W%R IS THE EBE MATRIX VECTOR PRODUCT
C     COR_INT IS DONE BUT WILL BE REDONE BELOW
C     EBE TERMS AT FREE SURFACE = -(ALL TERMS ON THE SAME VERTICAL)
...
!       RESULT SUBTRACTED TO MTRA1
        ...
        ENDDO
      ENDIF
C     RE-ASSEMBLING AFTER COMPLETION AT THE FREE SURFACE 
...
C     FORMUL = 'VECDIF *     VER'
C     IF(SIGMAG.OR.OPTBAN.EQ.1) FORMUL(7:7)='2' 
C     CALL VECTOR(COR_VER,'=',FORMUL,IELM3,1.D0,
C    *            FLUVER,FLUVER,FLUVER,PRESSURE,SVIDE,SVIDE,
C    *            MESH3D,MSK,MASKEL)     
