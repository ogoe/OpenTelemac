...
The default value for both velocity and \textit{k}-$\epsilon$ is 1. That value
...
chosen for \textit{U} and \textit{V} (keyword \telkey{SCHEME FOR ADVECTION OF
...
\begin{itemize}
\item \telkey{SOLVER FOR PPE} refer to subsection 6.5.1 below,

\item \telkey{MAXIMUM NUMBER OF ITERATIONS FOR PPE} refer to subsection 6.5.2
below,

\item \telkey{ACCURACY FOR PPE} refer to subsection 6.5.2 below,

\item \telkey{OPTION OF SOLVER FOR PPE} refer to subsection 6.5.1 below,

\item \telkey{PRECONDITIONING FOR PPE} refer to subsection 6.5.3 below.
\end{itemize}
...
only ways developed in \telemac{3D} consist in solving the linear systems by
