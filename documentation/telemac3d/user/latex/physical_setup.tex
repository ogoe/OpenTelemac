...
\textit{W} vertical velocity, ignoring the diffusion, advection and other
...
\telemac{3D} solves a \textit{W} vertical velocity equation which is similar to
the \textit{U} and \textit{V} equations, with the additional gravity term.


\section{Modelling turbulence}

the Reynolds numbers ($R=U.L/\upsilon$) reached for tidal
flows or in an estuary are excessively high and illustrate basically turbulent
flows (\textit{L}, the scale of eddies, for example, assumes the value of water
depth \textit{h} for a vertically homogeneous flow). For such a kind of flow,
...
dispersion (refer to Theoretical note [1]). The horizontal and vertical
...
The VISCOS subroutine will necessarily be programmed. Within that subroutine,
geometrical information, basic hydrodynamic information (water depth, velocity
components) and time are made available to the user.

That option theoretically aims at enabling the user to define the turbulent
viscosity by programming the VISCOS subroutine.
...
\begin{align}
\nu =L_{m}^{2} \sqrt{2D_{ij} D_{ij}} \text{ where }  D_{ij} =\frac{1}{2}
...
\item  5: Quetin. Better representation of wind drift. In windy weather, a
surface boundary layer is formed and viscosity decreases,
...
\item  1: user-performed; Law programming in the DRIUTIDRIUTI subroutine,
...
This algorithm is implemented in the subroutine VISCLM.
...
highly non-linear flow.


\subsection{k-$\epsilon$}

\telemac{3D} gives an opportunity to use the so-called \textit{k}-$\epsilon$
...
The \textit{k}-$\epsilon$ model is defined through a couple of equations
solving the balance equations for \textit{k} (turbulent energy) and $\epsilon$
(turbulent dissipation). Applying the \textit{k}-$\epsilon$ model often
...
\textit{k}-$\epsilon$ models, the user can refer to the \telemac{3D} Theoretical
Note.
...
The basic value of that viscosity may have to be increased to ensure a minimum
diffusion, especially upon the initial instants of the computation.
...
\begin{align}
\vec{\tau } = -\rho U^{*2} = -\frac{1}{2} \rho C_{f} \sqrt{U^{2} +V^{2} }
\vec{U}=\mu \frac{\partial \vec{U}}{\partial n}
\end{align}
...
\telkey{TURBULENCE MODEL FOR THE BOTTOM}. The available options are:
...
The friction is activated using the keyword \telkey{TURBULENCE MODEL FOR
...
\label{sec:srcfile}
tELEMAC-3D offers an opportunity to place momentum sources or sinks in any
point of the domain.
...
specific programming in the T3D\_DEBSCE subroutine (and T3D\_TRSCE in the
presence of tracer), or use the source file whose name is given by the keyword
\telkey{SOURCES FILE}. This file has exactly the same structure as the liquid
...
can be given using the couple of keywords \telkey{VELOCITIES OF THE SOURCES
ALONG X} and \telkey{VELOCITIES OF THE SOURCES ALONG Y}. Otherwise, the user
should program the SOURCE subroutine in order to amend USCE (for the velocity
at sources along \textit{X}) and VSCE (for the velocity at sources along
\textit{Y}). The user can use the time and all the parameters of the sources
...
tELEMAC-3D can be used to simulate flow while taking into account the influence
...
\begin{tabular}{ll}
if $\overrightarrow{U}_{vent} < 5$m/s & $a_{vent}$  = 0.565 10-3 \\
if $5 < \overrightarrow{U}_{vent}  < 19.22 $m/s & $a_{vent}$ = (- 0.12 + 0.137$\overrightarrow{U}_{vent}$ ) 10-3 \\
if $\overrightarrow{U}_{vent} > 19.22$ m/s & $a_{vent}$ = 2.513 10-3 \\
\end{tabular}

The parameter \telkey{COEFFICIENT OF WIND INFLUENCE} asked for by \telemac{3D}
is: avent (rair / r) and not only avent.  rair is approximately 1.2 kg/m3 and r
is approximately 1,000 kg/m3. Thus it is necessary to divide the value of
...
The METEO subroutine is provided to define the wind velocity and direction even
though they are space- or time-variable. The BORD3D subroutine describes the
law of wind-induced drift of water bodies.
\end{WarningBlock}
...
NO). The value of that pressure is directly set in the METEO subroutine.
By default, the latter initializes a pressure of 105 Pa ($\approx$ 1 atm) over
the whole domain.
...
direct programming in the BORD3D subroutine.
...
changed in the module EXCHANGE\_WITH\_ATMOSPHERE).
...
roughness length of \textit{z}${}_{0}$~=~0.0002~m has been chosen in the code,
that leads to \textit{U}${}_{2}$ $\approx$ 0.85\textit{U}${}_{10}$. This value
...
SOURCE\_TRAC subroutines. Two laws are suggested: the first one
uses the \textit{in situ} measurements of Secchi length and is
...
water column, the parameter \textit{b} that appears in the wind function is the
...
system (mainly TOMAWACTOMAWAC but also ARTEMISARTEMIS). In the current version,
...
case of TOMAWAC, the variables FX and FY,
...
\item  fill the keyword \telkey{RRECORD NUMBER IN WAVE FILE}. This value
...
Earth's rotational velocity of 7.27 x 10${}^{-5}$ rad/s and $\lambda$ is the
