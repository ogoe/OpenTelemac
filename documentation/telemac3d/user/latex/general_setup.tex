...
specified using the keywords \telkey{ORIGINAL DATE OF TIMEORIGINAL DATE OF
TIME} (AAAA, MM, JJ format; default value = 1900; 1; 1) and \telkey{ORIGINAL
HOUR OF TIMEORIGINAL HOUR OF TIME} (HH, MM, SS format, default value = 0; 0;
...
This latter choice requires the user to make changes in the CONDIM
subroutine:
...
computation if the keyword \telkey{INITIAL CONDITIONS} is set to 'PARTICULAR'
without any actual amendment of the subroutine.

The CONDIM subroutine successively initializes the two-dimensional
...
for \telemac{3D}, i.e. not only the co-ordinates of the \textit{X}, \textit{Y}
...
In \telemac{3D}, the water depth \textit{H}, the horizontal velocities \textit{U}
...
by \telemac{3D}, the indications in the boundary conditions file will then be
ignored.
...
insert the keyword \telkey{PRESCRIBED FLOWRATES} if there
...
In the case of options 2 to 6, the boundary conditions are imposed so that the
