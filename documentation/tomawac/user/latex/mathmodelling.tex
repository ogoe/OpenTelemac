...
\includegraphics*[width=4.25in, height=3.55in, keepaspectratio=false]{image95}
...
$\dot{x}=C_{g} .\sin \theta +U_{x} $ (4.12.a)
...
\paragraph{ Consideration of diffraction}
...
$\dot{\lambda }=\frac{1}{R\cos \varphi } \left(C_{g} .\sin \theta +U_{\lambda } \right)$ (4.25.a)
...
\subsection{ TOMAWAC source and sink terms}
...
\end{equation}
 ...
 In the Janssen's model [Janssen,~1991], the Miles' parameter b is a function of two non-dimensional variables:
...
\paragraph{ Bottom friction-induced dissipations (term Qbf)}
...
$\sigma _{3} =(1-\lambda )\sigma $ (4.61.d)
 ...
 \includegraphics*[width=4.46in, height=1.06in, keepaspectratio=false, trim=0.00in 0.00in 0.14in 0.00in]{image96}
...
\paragraph{ Wave blocking effects (upper limit for spectrum or dissipation Qds,cur)}
 ...
 \underbar{Option 2} : add a dissipative term on the right-hand side of the action balance equation (Westhuysen 2012)
...
{\bf  option 1 for wave blocking: upper limit for the spectrum}
 ...
 An upper limit \includegraphics*[width=1.75in, height=0.50in, keepaspectratio=false]{image97}$E(f)>E\max =\frac{\alpha g^{2} }{\left(2\pi \right)^{4} f^{5} } $$E(f)>E\max =\frac{\alpha g^{2} }{\left(2\pi \right)^{4} f^{5} } $ is imposed to the spectrum, using a Phillips (1977) shape, i.e.

 If

 with a the Phillips's constant in the Pierson-Moskowitz spectrum, equal to = 0.0081

 \includegraphics*[width=1.72in, height=0.46in, keepaspectratio=false]{image98}$F(f,\theta )=\frac{E\max }{E(f)} F(f,\theta )$$F(f,\theta )=\frac{E\max }{E(f)} F(f,\theta )$then
