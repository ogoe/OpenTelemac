...
complx = re.compile(r'(?P<n>(\d+|\([\d.+-dDeE]+?[;,][\d.+-dDeE]+?\)|[\[\(][\]\)])((?=[^#@])|[#@][\[\(].+?[\]\)]|[#@][^,;\[\(\)\]]+))',re.I)
...
def parseArrayGrid(s,size):
   ...
   # ~~ Identify individual grids ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   for minx,miny,maxx,maxy,nx,ny in re.findall(gridxyn,s+','):
      grids.append([ (float(minx),float(miny)),(float(maxx),float(maxy)),[int(nx),int(ny)] ])
