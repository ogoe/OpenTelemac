...
$INCDIR=$INCDIR." $cmdInc".$PROJECT.$ps."builds$ps".$dirlib.$ps."lib";
#
...
{#RunGredelpts
 ...
 #Lancement GREDELPTS, append outputs
  $command=join "",$PROJECT,$ps,"builds$ps$dirlib$ps","bin$ps","gredelpts$VERS[$0].exe < gredelpts.par >> gredelpts.log";
  $ret = system ("$command");
  return $ret;
}#RunGredelpts (end)
...
{#RunGredelseg
 ...
 #Lancement GREDELSEG, append outputs
  $command=join "",$PROJECT,$ps,"builds$ps$dirlib$ps","bin$ps","gredelseg$VERS[$0].exe < gredelseg.par >> gredelseg.log";
  $ret = system ("$command");
  return $ret;
}#RunGredelseg (end)
...
{#RunGredelmet
 ...
 #Lancement GREDELMET, append outputs
  $command=join "",$PROJECT,$ps,"builds$ps$dirlib$ps","bin$ps","gredelmet$VERS[$0].exe < gredelmet.par >> gredelmet.log";
  $ret = system ("$command");
  return $ret;
}#RunGredelmet (end)
...
{#RunGredelhyd
 ...
 #Lancement GREDELHYD, append outputs
  $command=join "",$PROJECT,$ps,"builds$ps$dirlib$ps","bin$ps","gredelhyd$VERS[$0].exe < gredelhyd.par >> gredelhyd.log";
  $ret = system ("$command");
  return $ret;
}#RunGredelhyd (end)
