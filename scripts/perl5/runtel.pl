...
#Localiser les modules dans le r̩pertoire 'bin'
  $project=`getproject`;  unshift (@INC, "$project$ps"."scripts$ps"."perl5");

#Verification des modules
  if ( ! -e "$project$ps"."scripts$ps"."perl5$ps"."tm_info.pm") 
       {die "FATAL : Module <tm_info> inaccessible.\n";}
  eval (require 'tm_info.pm'); 
  die "FATAL : Module <tm_info> incorrect ($@)"  if  ($@);
  if ( ! -e "$project$ps"."scripts$ps"."perl5$ps"."tm_casdico.pm") 
...
#----------------------------------
$filename3=join "", $PROJECT,$ps,"scripts$ps","perl5",$ps,"runcode.pl";
