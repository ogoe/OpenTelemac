...
!
! PARTEL_EXTENS FUNCTION
!
      CHARACTER(LEN=11) :: PARTEL_EXTENS
      EXTERNAL PARTEL_EXTENS   
!
!----------------------------------------------------------------------
!
...
!
      ...
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'IKLES')
      IF(NPLAN.GT.1) THEN
        ALLOCATE (IKLES3D(NELEM*NDP),STAT=ERR)
        IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'IKLES3D')
      ENDIF 
...
!
      ...
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'LIHBOR')
      ALLOCATE (LIUBOR(NPTFRMAX),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'LIUBOR')
      ALLOCATE (LIVBOR(NPTFRMAX),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'LIVBOR')
      ALLOCATE (HBOR(NPTFRMAX),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'HBOR')
      ALLOCATE (UBOR(NPTFRMAX),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'UBOR')
      ALLOCATE (VBOR(NPTFRMAX),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'VBOR')
      ALLOCATE (AUBOR(NPTFRMAX),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'AUBOR')
      ALLOCATE (TBOR(NPTFRMAX),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'TBOR')
      ALLOCATE (ATBOR(NPTFRMAX),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'ATBOR')
      ALLOCATE (BTBOR(NPTFRMAX),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'BTBOR')
      ALLOCATE (LITBOR(NPTFRMAX),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'LITBOR')
      ALLOCATE (NBOR(2*NPTFRMAX),STAT=ERR)  ! FOR FRONT2
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'NBOR')
      ALLOCATE (NUMLIQ(NPTFRMAX),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'NUMLIQ')
      ALLOCATE (NUMSOL(NPTFRMAX),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'NUMSOL')
      ALLOCATE (CHECK(NPTFRMAX),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'CHECK')
!
! CORE NAME LENGTH
!
      I_S  = LEN(NAMECLI)
...
!
      ...
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'DEJAVU')
      ALLOCATE (KP1BOR(NPTFR,2),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'KP1BOR')
!JAJ----------V ////
...
!     CAUSE: CALLING VOISIN WITH NELEM2 BUT IFABOR(NELEM=NELEM3,NDP=6)
      ...
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'IFABOR')
      ALLOCATE (IFANUM(NELEM2,3),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'IFANUM')
      ALLOCATE (IKLBOR(NPTFR,2),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'IKLBOR')
      ALLOCATE (NELBOR(NPTFR),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'NELBOR')
      ALLOCATE (NULONE(NPTFR,2),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'NULONE')
      ALLOCATE (ISEGF(NPTFR),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'ISEGF')
      ALLOCATE (IKLE(NELEM2,3),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'IKLE')
      ALLOCATE (IT1(NPOIN),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'IT1')
      ALLOCATE (IT2(NPOIN),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'IT2')
      ALLOCATE (IT3(NPOIN),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'IT3')
!
...
!        
     ...
     &       (NELEM2, NPOIN2, EPTR, EIND, NCOMMONNODES, NUMFLAG,
     &        PTXADJ, PTADJNCY)
...
!
     ...
     &                           NUMFLAG,             ! BASEVAL
     &                           NELEM2,              ! VERTNBR
     &                           XADJ2(1:NELEM2),     ! VERTTAB
     &                           XADJ2(2:NELEM2+1),   ! VENDTAB 
     &                           XADJ2(1:NELEM2),     ! VELOTAB, VERTEX WEIGHTS (nULL)
     &                           XADJ2(1:NELEM2),     ! VLBLTAB, VERTEX LABELS (nULL)
     &                           XADJ2(NELEM2+1)-1,   ! EDGENBR 
     &                           ADJNCY2,             ! EDGETAB 
...
!
        ! RUN PARTITIONING
        WRITE(LU,*) ' THE MESH PARTITIONING STEP SCOTCH STARTS'
...
!      
      ...
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'NELEM_P')
       
      ALLOCATE (NPOIN_P(NPARTS),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'NPOIN_P')
      
      ALLOCATE (NPTFR_P(NPARTS),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'NPTFR_P')
      
      ALLOCATE (NPTIR_P(NPARTS),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'NPTIR_P')
      
      ALLOCATE (NHALO(NPARTS),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'NHALO')

      ALLOCATE(TAB_TMP( NBMAXNSHARE),STAT=ERR)
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'TAB_TMP')
...
!     E IS THE LOCAL LABEL ON SUBMESH I 
      ...
      IF (ERR.NE.0) CALL PARTEL_ALLOER (LU, 'ELELG')
      ELELG(:,:)=0
...
!
        ...
        IF (ERR/=0) CALL PARTEL_ALLOER (LU, 'CHAIN(ISEC)%LISTE') 
...
!     CALL EXIT(ICODE)
      END SUBROUTINE PARTEL_PLANTE2
C
C                       ****************************************
                        CHARACTER(LEN=11) FUNCTION PARTEL_EXTENS
C                       ****************************************
C
     *(N,IPID)
C
...
C***********************************************************************
C
C      FONCTIONS: PARTEL_EXTENSION DES FICHIERS SUR CHAQUE PROCESSEUR.
C      ==========
C
C-----------------------------------------------------------------------
C                             ARGUMENTS
...
C
      IF(N.GT.0) THEN
C
        PARTEL_EXTENS='00000-00000'
C
        IF(N.LT.10) THEN
          WRITE(PARTEL_EXTENS(05:05),'(I1)') N
        ELSEIF(N.LT.100) THEN
          WRITE(PARTEL_EXTENS(04:05),'(I2)') N
        ELSEIF(N.LT.1000) THEN
          WRITE(PARTEL_EXTENS(03:05),'(I3)') N
        ELSEIF(N.LT.10000) THEN
          WRITE(PARTEL_EXTENS(02:05),'(I4)') N
        ELSE
          WRITE(PARTEL_EXTENS(01:05),'(I5)') N
        ENDIF
C
        IF(IPID.LT.10) THEN
          WRITE(PARTEL_EXTENS(11:11),'(I1)') IPID
        ELSEIF(IPID.LT.100) THEN
          WRITE(PARTEL_EXTENS(10:11),'(I2)') IPID
        ELSEIF(IPID.LT.1000) THEN
          WRITE(PARTEL_EXTENS(09:11),'(I3)') IPID
        ELSEIF(IPID.LT.10000) THEN
          WRITE(PARTEL_EXTENS(08:11),'(I4)') IPID
        ELSE
          WRITE(PARTEL_EXTENS(07:11),'(I5)') IPID
        ENDIF
C
      ELSE
C
...
900     FORMAT(1X,'PARTEL_ELEBD : IELM=',1I6,' TYPE D''ELEMENT INCONNU')
901     FORMAT(1X,'PARTEL_ELEBD: IELM=',1I6,' UNKNOWN TYPE OF ELEMENT')
        CALL PARTEL_PLANTE2(-1)
        STOP
      ENDIF
C
...
C        PARTEL_ALLOER, PARTEL_ALLOER2 (GESTION MSGS)
...
!
      ...
      CHARACTER(LEN=11), EXTERNAL :: PARTEL_EXTENS ! PARTEL_EXTENSION DES NOMS FICHIER
...
!     We read a line, locate the colon ':' and then read the number.
      ...
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' PRIORITY')
      WRITE(LU,92) NPOINT
...
CD    ****************************** ALLOCATION MEMORY ADDED BY CD
      ...
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,'NELEM_P')
      ALLOCATE(NPOIN_P(NPARTS),STAT=IERR)
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,'NPOIN_P')
      ALLOCATE(NODELG(NPOINT),STAT=IERR)
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,'NODELG')
CD    *******************************      
      
      ALLOCATE(X1(NPOINT),STAT=IERR)
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' X1')
      ALLOCATE(Y1(NPOINT),STAT=IERR)
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' Y1')
      ALLOCATE(Z1(NPOINT),STAT=IERR)
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' Z1')
      ALLOCATE(NCOLOR(NPOINT),STAT=IERR)
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' NCOLOR')
      ALLOCATE(NCOLOR2(NPOINT),STAT=IERR)
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' NCOLOR2')
      ALLOCATE(ECOLOR(NELEMTOTAL),STAT=IERR)
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' ECOLOR')
      ALLOCATE(IKLESTET(4*NELEMTOTAL),STAT=IERR)
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' IKLESTET')
      ALLOCATE(IKLESTRI(3*NELEMTOTAL),STAT=IERR)
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' IKLESTRI')
      ALLOCATE(IKLESTRIN(NELEMTOTAL,4),STAT=IERR)
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' IKLESTRIN')
      ALLOCATE(TYPELEM(NELEMTOTAL,2),STAT=IERR)
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' TYPELEM')
      ALLOCATE(CONVTRI(NELEMTOTAL),STAT=IERR)
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' CONVTRI')
      ALLOCATE(CONVTET(NELEMTOTAL),STAT=IERR)
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' CONVTET')
      ALLOCATE(NPOINTSD(NPARTS),STAT=IERR)
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' NPOINTSD')
      ALLOCATE(NELEMSD(NPARTS),STAT=IERR)
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' NELEMSD')
      ALLOCATE(NPOINTISD(NPARTS),STAT=IERR)
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' NPOINTISD')

!F.D
      ALLOCATE(NBOR2(NPOINT),STAT=IERR)
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' NBOR2')
      ALLOCATE(TEMPO(2*NPOINT),STAT=IERR)
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' TEMPO')
      ALLOCATE(FACE_CHECK(NBFAMILY),STAT=IERR)
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' FACE_CHECK')      
      ALLOCATE(GLOB_2_LOC(NPOINT),STAT=IERR)
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' GLOB_2_LOC')
      ALLOCATE(IKLES(NELEMTOTAL,4),STAT=IERR)
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' IKLES')
...
!---------------
      ...
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' NELBOR')
      ALLOCATE(NULONE(NBTRI,3),STAT=IERR)
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' NULONE')
      ALLOCATE(IKLBOR(NBTRI,3),STAT=IERR)
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' IKLBOR')
      ALLOCATE(IKLE(NBTET,4),STAT=IERR)
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' IKLE')
      ALLOCATE(IFABOR(NBTET,4),STAT=IERR)
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' IFABOR')
!
!------------------------------------------------------- DEBUT VERSION O. BOITEAU
! RECHERCHE DE LA CORRESPONDANCE TETRAEDRE <--> TRIANGLES DE BORDS
!      ALLOCATE(TETTRI(4*NBTET),STAT=IERR)
!      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' TETTRI')
!      ALLOCATE(TETTRI2(NBTET),STAT=IERR)
!      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' TETTRI2')
!      TETTRI(:)=-1
!      TETTRI2(:)=0
!      DO I=1,NBTRI
!        N=3*(I-1)+1
...
!
      ...
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' IKLEIN')
!
      DO IELEM = 1, NELIN
         DO I = 1,4
...
!      
      ...
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' NBOR')
!
      DO IELEM = 1, NPTFR
...
!
      ...
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' TRI_REF')
       TRI_REF=0 
       DO J = 1, NBTRI
         K = 3*(J-1)+1  
         IKLE1 = IKLESTRI(K)
...
! LES REFERENCE
      ...
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' NODES1')
      NODES1(:)=0
      DO I=1,NBTET
        DO K=1,4
...
! DE NODES3(NODES2(I)) A NODES3(NODES2(I)+NODES1(I)-1)
      ...
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' NODES2')
      COMPT=0
      NODES2(1)=1
      DO I=1,NPOINT
...
! POUR UN NOEUDS DONNE, QU'ELLES SONT LES MAILLES QUI LE CONCERNENT
      ...
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' NODES3')
      NODES3(:)=-1
      DO I=1,NBTET
        DO K=1,4
          IKLEB=IKLESTET(4*(I-1)+K)
...
!---------------
      ...
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' NODES1T')
      NODES1T(:)=0
      DO I=1,NBTRI
        DO K=1,3
      ...
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' NODES2T')
      COMPT=0
      NODES2T(1)=1
      DO I=1,NPOINT
      ...
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' NODES3T')
      NODES3T(:)=-1
      DO I=1,NBTRI
        DO K=1,3
          IKLEB=IKLESTRI(3*(I-1)+K)
...
CD     ELEGL IS THE FILLED 
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,'ELEGL')
      DO I=1,NBTET
         NELEM_P(EPART(I))=NELEM_P(EPART(I))+1
...
!
      ALLOCATE(NODEGL(MAX_NPOIN_P,NPARTS),STAT=IERR)
       IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,'NODEGL')
       DO IDD=1,NPARTS
          NODELG(:)=0
          DO POS=1,NELEM_P(IDD)
             IELEM=ELEGL(POS,IDD)
...
!------------
      ...
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' NODES4')
C$$$      NODES4(:)=-1
      ALLOCATE(KNOLG(NPOINT),STAT=IERR)      ! C'EST SOUS-OPTIMAL EN
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' KNOLG')! TERME DE DIMENSIONNEMENT
...
!      
      ...
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,' NACHB')
      NACHB(1,:)=0
      DO J=2,NBSDOMVOIS-1
        NACHB(J,:)=-1
      ENDDO
      ALLOCATE(TRIUNV(4*NBTRI),STAT=IERR)
      IF (IERR.NE.0) CALL PARTEL_ALLOER (LU, 'TRIUNV')
!
!
...
!
      ...
      IF (IERR.NE.0) CALL PARTEL_ALLOER(LU,'DEJA_TROUVE')
      DEJA_TROUVE(:)=.FALSE.
!      
...
! NOM DU FICHIER UNV PAR SOUS-DOMAINE
        NAMEINP2(I_LENINP+1:I_LENINP+11) = PARTEL_EXTENS(NPARTS-1,IDD-1)
        OPEN(NINP2,FILE=NAMEINP2,STATUS='UNKNOWN',FORM='FORMATTED',
     &       ERR=132)
        REWIND(NINP2)

! NOM DU FICHIER LOG PAR SOUS-DOMAINE
        NAMELOG2(I_LENLOG+1:I_LENLOG+11) = PARTEL_EXTENS(NPARTS-1,IDD-1)
...
! END OF FILE AND FORMAT ERRORS :
 ...
 1200 TEXTERROR='ERREUR DE FIN DE LECTURE DU FICHIER UNV '//
  ...
  999 CALL PARTEL_ALLOER2(LU,TEXTERROR)
!
      END SUBROUTINE PARES3D
!
!                       *************************
                        SUBROUTINE PARTEL_ELEBD31
!                       *************************
!
...
101              FORMAT(1X,'PARTEL_ELEBD31 : ERREUR DANS LE MAILLAGE ')
102              FORMAT(1X,'PARTEL_ELEBD31 : ERROR IN MESH. BYE.')
                 CALL PARTEL_PLANTE2(1)
                 STOP
               END IF
               NELBOR(IELEB)   = IELEM
...
!
      RETURN
      END SUBROUTINE PARTEL_ELEBD31   
! 
!                       **************************
                        SUBROUTINE PARTEL_VOISIN31
!                       **************************
!
     *(NBTET,NELMAX,SIZIKL,
