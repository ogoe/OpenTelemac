...
!>   </td></tr>
!>     <tr><th> Internal(s)
!>    </th><td> A1, C, CH, CHARACTERISTIC, CH_BLENGTH, CH_DELTA, CH_TYPES, DELTAZ, DENOM, DOIT, DX1, DXP, DY1, DYP, DZP, EPM1, EPSDZ, EPSI, EPSILO, ETAS, HEAPCHAR, HEAPCOUNTS, I, I1, I2, I3, IBASE, ICHA, IEL, IELE, IER, IET, IET2, IFA, IGEN, II, III, ILB, ILOC, INIT, INTEX, IPLOT, IPOIN, IPROC, ISO, ISOH, ISOV, ISP, ISTOP, ISUI, ISUI2, IUB, J, K, LAST_NOMB, MAX_BASKET_SIZE, MPI_COMM_WORLD, MPI_INTEGER, MPI_REAL8, MPI_SUCCESS, MPI_UB, N, NARRV, NBPTS2, NCHARA, NCHDIM, NEPID, NLOSTAGAIN, NLOSTCHAR, NPOINT, NPOINT2, NRK, NSEND, NSPMAX, PAS, PAS2, QUAD, RDISPLS, RECVCHAR, RECVCOUNTS, SDISPLS, SENDCHAR, SENDCOUNTS, STEST, TRACE, XP, YP, ZP
!>   </td></tr>
!>     </table>
...
!>  <br><table>
!>     <tr><th> Known(s)
!>    </th><td> ADD_CHAR11(), ADD_CHAR41(), ADD_CHAR41_MOD(), CHGDIS(), COLLECT_CHAR(), DEORG_CHARAC_TYPE(), GET_MPI_PARAMETERS(), GLOB_CHAR_COMM(), GTSH11(), GTSH13(), GTSH41(), HEAP_FOUND(), INTERP(), INTERP_RECVCHAR_11(), INTERP_RECVCHAR_41(), INTERP_RECVCHAR_41_MOD(), INTRODUCE_RECVCHAR(), INTRODUCE_RECVCHAR_MOD(), ORGANISE_CHARS(), ORG_CHARAC_TYPE(), OV(), PARCOM(), PLANTE(), PREP_INITIAL_SEND(), PREP_LOST_AGAIN(), PREP_SENDBACK(), PRINT_HEAPCHAR(), PRINT_RECVCHAR(), PRINT_SENDCHAR(), P_IMAX(), P_ISUM(), P_MPI_ADDRESS(), P_MPI_ADDRESS2(), P_MPI_ADDRESS3(), RE_INITIALISE_CHARS(), SCHAR11(), SCHAR41(), WIPE_HEAPED_CHAR()
!>   </td></tr>
!>     <tr><th> Unknown(s)
!>    </th><td> CHARACTERISTIC, CHARACTERISTIC, CHARACTERISTIC, CH_BLENGTH, CH_DELTA, CH_TYPES, HEAPCHAR, HEAPCHAR, HEAPCHAR, HEAPCHAR, HEAPCHAR, HEAPCHAR, HEAPCHAR, HEAPCOUNTS, HEAPCOUNTS, HEAPCOUNTS, HEAPCOUNTS, HEAPCOUNTS, HEAPCOUNTS, HEAPCOUNTS, ICHA, ICHA, ICHA, ICHA, ICHA, MAX_BASKET_SIZE, MAX_BASKET_SIZE, MAX_BASKET_SIZE, MPI_ALLTOALL, MPI_ALLTOALLV, MPI_COMM_WORLD, MPI_COMM_WORLD, MPI_COMM_WORLD, MPI_INTEGER, MPI_INTEGER, MPI_INTEGER, MPI_INTEGER, MPI_REAL8, MPI_REAL8, MPI_REAL8, MPI_SUCCESS, MPI_SUCCESS, MPI_SUCCESS, MPI_TYPE_COMMIT, MPI_TYPE_EXTENT, MPI_TYPE_FREE, MPI_TYPE_LB, MPI_TYPE_STRUCT, MPI_TYPE_UB, MPI_UB, MPI_UB, MPI_UB, P_IMAX, P_IMAX, P_ISUM, RDISPLS, RDISPLS, RDISPLS, RECVCHAR, RECVCHAR, RECVCHAR, RECVCHAR, RECVCHAR, RECVCHAR, RECVCHAR, RECVCHAR, RECVCHAR, RECVCHAR, RECVCHAR, RECVCHAR, RECVCHAR, RECVCOUNTS, RECVCOUNTS, RECVCOUNTS, RECVCOUNTS, RECVCOUNTS, SDISPLS, SDISPLS, SDISPLS, SDISPLS, SDISPLS, SDISPLS, SENDCHAR, SENDCHAR, SENDCHAR, SENDCHAR, SENDCHAR, SENDCHAR, SENDCHAR, SENDCOUNTS, SENDCOUNTS, SENDCOUNTS, SENDCOUNTS, SENDCOUNTS, SENDCOUNTS, SENDCOUNTS, TRACE, TRACE, TRACE, TRACE, TRACE, TRACE, TRACE, TRACE
!>   </td></tr>
!>     </table>
...
C        PUBLIC :: SCARACT     ! CALLED IN (MODIFIED) CHARAC OF TELEMAC2D FROM BIEF
        PUBLIC :: INIT_TOMAWAC
...
C         ORIGINAL CODE:
C
C         CALL MPI_ADDRESS (CH%MYPID,  CH_DELTA(1),  IER)
C         CALL MPI_ADDRESS (CH%NEPID,  CH_DELTA(2),  IER)
C         CALL MPI_ADDRESS (CH%INE,    CH_DELTA(3),  IER)
C         CALL MPI_ADDRESS (CH%KNE,    CH_DELTA(4),  IER)
C         CALL MPI_ADDRESS (CH%IOR,    CH_DELTA(5),  IER)
C         CALL MPI_ADDRESS (CH%ISP,    CH_DELTA(6),  IER)
C         CALL MPI_ADDRESS (CH%NSP,    CH_DELTA(7),  IER)
C         CALL MPI_ADDRESS (CH%XP,     CH_DELTA(8),  IER)
C         CALL MPI_ADDRESS (CH%YP,     CH_DELTA(9),  IER)
C         CALL MPI_ADDRESS (CH%ZP,     CH_DELTA(10), IER)
C         CALL MPI_ADDRESS (CH%BASKET, CH_DELTA(11), IER) ! BASKET STATIC
C
  ...
  !---------------------------------------------------------------------
  ! GETS/SETS ON DIMENSIONS AND COUNTERS
  !---------------------------------------------------------------------
...
C         MAYBE NOT MANDATORY
  ...
  !---------------------------------------------------------------------
     ...
     &       ' @STREAMLINE::GLOB_CHAR_COMM::MPI_ALLTOALL ERROR: ',IER
            CALL PLANTE(1)
          ENDIF
          RDISPLS(1) = 0 ! SAVES THE RECEIVED DATA CONTIGUOUSLY
          DO I=2,NCSIZE
...
C           END OF THIS IS JUST TO AVOID A BUG ON HP COMPILER
  ...
  !---------------------------------------------------------------------
        ...
        SUBROUTINE INTRODUCE_RECVCHAR_MOD(VAL,NPOIN,NARRV)
          ...
          DOUBLE PRECISION, INTENT(INOUT) :: VAL(NPOIN)
          INTEGER I
C          IF (N>NOMB.OR.N>MAX_BASKET_SIZE) THEN
C            WRITE(LU,*)
C     &       ' @STREAMLINE::INTRODUCE_RECVCHAR ::',
C     &       ' N>NOMB.OR.N>MAX_BASKET_SIZE,',
C     &       ' N,NOMB,MAX_BASKET_SIZE : ',N,NOMB,MAX_BASKET_SIZE
C            CALL PLANTE(1)
C            STOP
C          ENDIF
          DO I=1,NARRV
...
!
C 08/11/04: ADAPTED TO THE GENERALISED SIGMA TRANSFORM
C 12/10/05: BUG CORRECTED, VARIABLE IELE WHICH WAS BEFORE IEL
C            AND ERASED ANOTHER IEL.
!
!
!***********************************************************************
!
...
!
!-----------------------------------------------------------------------
C  HERE: THE EXIT FACE IS A LIQUID BOUNDARY (CASE 0)
C  ENDS TRACING BACK (SIGN OF ELT)
C  OR, INTERSECTS WITH A PLANE AND RE-COMPUTATION OF THE
C  VELOCITIES IS REQUIRED (CASE 2)
!-----------------------------------------------------------------------
!
              IF(ABS(DZ(IPLOT)).GT.EPSDZ) THEN
...
!
!-----------------------------------------------------------------------
C  HERE: THE EXIT FACE BELONGS TO THE PLANE WHERE RE-COMPUTATION OF
C  THE VELOCITIES IS REQUIRED
!-----------------------------------------------------------------------
!
...
!
!-----------------------------------------------------------------------
C FILLS ELT,NSP,XPLOT,YPLOT, COMPUTE VALID SHP FUNCTIONS, RANGE 1..NPLOT
...
C JAJ PINXIT BASED ON CHAR11 FRI JUL 18 14:30:18 CEST 2008
!
!BD_INCKA ADAPTED FOR TOMAWAC COW PARALLELISATION 6/05/09
!                       *********************
...
!
      ...
      DOUBLE PRECISION, INTENT(INOUT) :: DX(NPLOT),DY(NPLOT),TEST(NPLOT)
      DOUBLE PRECISION, INTENT(INOUT) :: DZ(NPLOT)
C      INTEGER         , INTENT(IN)    :: IBOR(NELEM2,5,NPLAN-1)
...
!
      DO IPLOT=1,NPLOT
        ...
        ELT(IPLOT)     = RECVCHAR(IPLOT)%INE
        ETA(IPLOT)     = RECVCHAR(IPLOT)%KNE
        NSP(IPLOT)     = RECVCHAR(IPLOT)%NSP ! R-K STEPS TO BE FULFILLED
        ISPDONE(IPLOT) = RECVCHAR(IPLOT)%ISP ! R-K STEPS ALREADY DONE
        IEL = ELT(IPLOT)
        IET = ETA(IPLOT)
        XP  = XPLOT(IPLOT)
...
!
                  ...
                  ELSE
                    ...
                    SHP2(IPLOT) = 0.D0
                  ENDIF
C                     SHP(          IFA  ,IPLOT) = 1.D0 - A1
C                     SHP(     ISUI(IFA) ,IPLOT) = A1
C                     SHP(ISUI(ISUI(IFA)),IPLOT) = 0.D0
                     XPLOT(IPLOT) = X(I1) + A1 * DX1
                     YPLOT(IPLOT) = Y(I1) + A1 * DY1
!
                     ISO = ISOV
...
!
C  HERE: THE EXIT FACE IS A SUB-DOMAIN INTERFACE
C  INTERFACE POINT WILL BE TRATED IN THE NEXT SUB-DOMAIN
C  ONLY SETS TEST TO ZERO HERE
!-----------------------------------------------------------------------
!
!>>>>
...
!
C     THE ADVECTION FIELD U IS ASSUMED NOT TO VARY WITH TIME
C     THE DERIVATIVE IS ASSUMED TO BE PUNCTUAL HENCE NON DISPERSIVE
!
!-----------------------------------------------------------------------
C                             ARGUMENTS
! .________________.____.______________________________________________.
...
C  MODE: -->(DONNEE NON MODIFIEE),(DONNEE MODIFIEE)
!-----------------------------------------------------------------------
C CALLED BY : CARACT , DERIVE , DERLAG
C CALLS     : --
!
!***********************************************************************
!
...
!
!-----------------------------------------------------------------------
C HERE: DETECTS PASSING TO THE NEIGHBOUR SUBDOMAIN AND COLLECTS DATA
!-----------------------------------------------------------------------
!
...
C                            I3 ???
     ...
     &                     -(Y(I2)-Y(I1))*(XP-X(I1)))*SURDET(IEL)
          ...
          IF (IEL==-2) THEN  ! A LOST-AGAIN TRACEBACK DETECTED, ALREADY HERE!
            ! SETS THE IMPLANTING PARAMETERS
            IPROC=IFAPAR(IFA  ,ELT(IPLOT))
            ILOC =IFAPAR(IFA+3,ELT(IPLOT))
...
!
      ...
      IF(INIT) THEN ! CHECKS THINGS ONCE AND FOR ALL
!
...
!
C  TEMPORARY
C  TEST = NUMBER OF SUB-DOMAINS WHICH HAVE DEALT WITH A POINT
!
C  ADJUSTS SHP DEPENDING ON WHAT'S BEEN FOUND IN THE OTHER
C  SUB-DOMAINS (SEE 'TEST')
!
...
C         HENCE IT WILL NOT BE THIS ONE
        ...
        ENDDO
!
C       ADDS TEST OF NEIGHBOURING SUB-DOMAINS
C       WILL ALSO BE USED BY WIPE_CHAR...
        CALL PARCOM(STEST,2,MESH)
!
        DO IPOIN = 1,NPOINT
          IF(TEST(IPOIN).GT.1.5D0) THEN
C           SEVERAL SUBDOMAINS HAVE SUCCEEDED (ARRIVAL ON A BORDER)
C           THE AVERAGE VALUE WILL BE TAKEN
...
C            WRITE(*,*)'HERE CALL ADD_CHAR.. WILL BE USEFUL',IPOIN
          ENDIF
        ENDDO
!
      DEALLOCATE(STEST%R)
...
C ARE COMPLETED (I.E. THEIR FEET LOCATED) SO THAT
...
!
      IF(NCSIZE.GT.1) THEN
        ...
        ! TAKES INTO ACCOUNT THE JMH METHOD -> WIPES OUT FROM THE LIST
     ...
     &   (' ===> HEAPCHAR COLLECTED FOR THE TREATMENT ::',NOMB,NCHARA) ! DEBUG
        ...
        IF(P_IMAX(NLOSTCHAR)>0) THEN ! THERE ARE -REALLY- LOST TRACEBACKS SOMEWHERE
          ...
          ! PREPARES INITIAL SENDING OF COLLECTED LOST TRACEBACKS
     ...
     &     (' ===> SENDCHAR PREPARED BY TESTING ::',NOMB,NSEND) ! DEBUG
          ...
          DO
            ...
            ! GETS THE ARRIVING TRACEBACKS VIA ALL-TO-ALL COMMUNICATION
            CALL GLOB_CHAR_COMM()

            ! COMPUTES THE NUMBER OF ARRIVED ONES (NECESSARY)
     ...
     &             (' ===> RECVCHAR RECEIVED IS ::',NOMB,NARRV) ! DEBUG
             ...
             ! CALLS TRACKING WITH ARRIVED TRACEBACKS ...
             ! RE-USES THE PREVIOUSLY USED FIELDS FOR ADDITIONAL TRACEBACKS
     ...
     &           NOMB,NARRV) ! DEBUG
!
C             ! INTERPOLATES THE -LOCATED- TRACEBACKS -> SOME OF RANGE 1:NARRV
C             ! APPLYING THE JUST VALID ELT, SHP, ETC. JUST SAVED FROM
C             ! ADD_CHAR11 OR ADD_CHAR41 COMPUTED FOR JUST THIS RANGE
!             !
C             ! BEWARE: THIS IS THE ONLY REASON TO DO IT NOW -
C             ! WHEN THE ELT,SHP COULD BE SAVED, THIS COULD HAVE BEEN DONE
C             ! AFTER THE GENERATION LOOP WORKING ON HEAPCHAR -
C             ! WHICH WOULD HAVE BEEN MORE PERFORMANT, BECAUSE IN RECVCHAR
C             ! WE HAVE TO CHECK FOR EACH TRACEBACK IF IT IS LOCATED OR NOT
!             !
C             ! BEWARE: UTILD==RECVCHAR%BASKET(:)
              CALL INTERP_RECVCHAR_41_MOD(U%R,IKLE,ELT,ETA,SHP1,
     &                 SHP2,SHP3,SHZ,
     &                 NELEM,NPOIN2,NPLAN,NARRV)
!
C           IF(NARRV.GT.0) THEN
C             IF(U%TYPE==2) THEN
C                 CALL INTERP_RECVCHAR_41
C      &         (U%R,1,IKLE,ELT,ETA,SHP,SHZ,NELMAX,NPOIN2,NPLAN,NARRV)
C             ELSEIF(U%TYPE==4) THEN
C               IF(IELM.EQ.11) THEN
C                 DO I=1,NOMB
C                   CALL INTERP_RECVCHAR_11
C      &          (U%ADR(I)%P%R,I,IKLE,ELT,SHP,NELMAX,
C      &           U%ADR(I)%P%DIM1,NARRV,U%ADR(I)%P%ELM)
C                 ENDDO
C               ELSEIF(IELM.EQ.41) THEN
C                 DO I=1,NOMB
C                   CALL INTERP_RECVCHAR_41
C      &              (U%ADR(I)%P%R,I,IKLE,ELT,ETA,SHP,SHZ,
C      &                 NELMAX,NPOIN2,NPLAN,NARRV)
C                 ENDDO
C               ELSE
C                 WRITE(LU,*) 'WRONG IELM IN SCARACT:',IELM
C                 CALL PLANTE(1)
C                 STOP
C               ENDIF
C             ENDIF
...
C !       , SENDCHAR, RECVCHAR
     ...
     &        ' #LOST-AGAIN SUM EVERYWHERE: ', P_ISUM(NLOSTAGAIN)
             ...
             IF(P_ISUM(NLOSTAGAIN)>0) THEN ! THERE ARE LOST-AGAINS SOMEWHERE
               CALL PREP_LOST_AGAIN(NOMB,NSEND,NLOSTAGAIN,NARRV) ! PREPARES SENDING LOST-AGAINS
             ELSE
...
!
C           ! PREPARES SENDING OF THE HEAPED LOCALISED TRACEBACKS
C           CALL PREP_SENDBACK(NOMB,NCHARA)
C           ! THE FINAL SEND/RECV OF THE LOST TRACEBACKS BACK VIA ALLTOALL COMM.
           CALL GLOB_CHAR_COMM()
...
C             STOP
           ENDIF
!
C           ! INTRODUCES THE VALUES FROM THE RECEIVED TRACEBACK BASKETS
C           ! NOTE THAT IN 3D, NPOIN->NPOIN3 / WE DO NOT DISTINGUISH 11 AND 41
!
           IF (NARRV>0) THEN
              CALL INTERP_RECVCHAR_41_MOD(U%R,IKLE,ELT,ETA,SHP1,
     &                 SHP2,SHP3,SHZ,
...
C             IF(UTILD%TYPE==2) THEN
C               CALL INTRODUCE_RECVCHAR(UTILD%R,1,UTILD%DIM1,NOMB,NARRV)
C             ELSEIF(UTILD%TYPE==4) THEN
C               DO I=1,NOMB
C                 CALL INTRODUCE_RECVCHAR(UTILD%ADR(I)%P%R,I,
C      &                                  UTILD%ADR(I)%P%DIM1,
C      &                                  NOMB,NARRV)
C               ENDDO
C             ELSE
C               WRITE (LU,*)
C      &          ' @STREAMLINE::SCARACT :: UTILD%TYPE: ',UTILD%TYPE
C               CALL PLANTE(1)
C               STOP
C             ENDIF
           ENDIF
        ENDIF
         CALL RE_INITIALISE_CHARS(NSEND,NLOSTCHAR,NLOSTAGAIN,NARRV) ! DEALLOCATES
! !
...
C           ELSEIF(UTILD%TYPE==4) THEN
C           DO I=1,NOMB
C             CALL PARCOM(UTILD%ADR(I)%P,1,MESH)
C           ENDDO
C           ENDIF
         ENDIF
!
...
C     FUNCTION:
!
C     SOLVES THE ADVECTION EQUATIONS USING THE CHARACTERISTICS METHOD
C     FOR A SET OF FUNCTIONS.
!
C     BEWARE: COMPATIBILITY WITH 3.0 IS NO LONGER ENSURED BECAUSE
C     OF THE CALL TO PARCOM
!
C     HOWEVER U AND UTILD CAN NOW BE VECTORS,
C     IN THIS CASE NOMB WILL BE CONSIDERED EQUAL TO 1
!
...
!
      LOGICAL :: INIT=.TRUE.
      LOGICAL, PARAMETER :: DOIT=.TRUE. ! SWITCHES ON/OFF JMH METHOD
      LOGICAL QUAD
C PRE-NUMBER OF INITIALLY COLLECTED LOST CHARACTERISTICS
C WHICH IS LATER REDUCED BY APPLYING JMH'S PROCEDURE
...
!
!***********************************************************************
C NUMBER OF RUNGE KUTTA SUB-TIME STEPS PER CROSSED ELEMENT
!
      NRK = 3
!
      QUAD = .FALSE.
...
!
     ...
     &                MSK , MASKEL )
         ENDIF
!
C      CALLS THE SUBROUTINE THAT TRACES BACK THE CHARACTERISTICS CURVES
!
...
C
C         TRIANGLES P2 FOR ONE OF THE ADVECTED VARIABLES
C         ===============================================
C
C         FILLS SHP AND ELT (OPTIMISED)
C
...
C
C         CASE NOT ENVISAGED: QUADRATIC TRACER AND LINEAR VELOCITY
C
          IF(IELMU.NE.13)THEN
...
!
C         VECTORS U AND UTILD (NOMB EQUALS 1)
!
          IF(U%ELM.EQ.13) THEN
C           INTERPOLATES VELOCITIES FOR A QUADRATIC VARIABLE
            CALL INTERP(U%R,UTILD%R,SHP,NDP,SHZ,ETA,ELT,
     &                  U%DIM1,U%DIM1,NPLAN,13,IKLE,NELMAX)
          ELSE
C           INTERPOLATES VELOCITIES IN OTHER CASES
            CALL INTERP(U%R,UTILD%R,SHP,NDP,SHZ,ETA,ELT,
...
!
     ...
     &          'THE NUMBER OF TRACEBACK INTERFACE CROSSINGS IGEN > 99'
          ...
          ! PREPARES SENDING OF THE HEAPED LOCALISED TRACEBACKS
     ...
     &                                 NLOSTCHAR, NARRV
          ...
          ! INTRODUCES THE VALUES FROM THE RECEIVED TRACEBACK BASKETS
