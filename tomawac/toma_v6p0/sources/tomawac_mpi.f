...
C
     ...
     &              ISO(IPLOT)=IBSET(ISO(IPLOT),3)
C
C               IF ((IPLOT==5736).AND.(IPID==1)) THEN
C                    WRITE(*,*)
C      *            'ISO 1434,3PLAN',ISO(IPLOT),'GOODELT',GOODELT(IPLOT),
C      *        'DX',DX(IPLOT),'DY',DY(IPLOT),'SHP1',
C      *        SHP1(IPLOT),'SHP2',SHP2(IPLOT),'SHP3',SHP3(IPLOT),'NSP',
C      *        NSP(IPLOT),'XP',XPLOT(IPLOT) - DX(IPLOT),'YP',
C      *        YPLOT(IPLOT)-DY(IPLOT)
C               ENDIF
C               IF ((IPLOT==5597).AND.(IPID==0)) THEN
C                    WRITE(*,*)
C      *           'ISO 1361,3 PLAN',ISO(IPLOT),'GOODELT',GOODELT(IPLOT),
C      *        'DX',DX(IPLOT),'DY',DY(IPLOT),'SHP1',
C      *        SHP1(IPLOT),'SHP2',SHP2(IPLOT),'SHP3',SHP3(IPLOT),'NSP',
C      *        NSP(IPLOT),'XP',XPLOT(IPLOT) - DX(IPLOT),'YP',
C      *        YPLOT(IPLOT)-DY(IPLOT)
...
!
!
C               IF (GOODELT(IPLOT) == 1100) THEN
C                  ISO(IPLOT) = 1
C               ENDIF
C               IF ((IPLOT==18291).AND.(IPID==6)) WRITE(*,*)
C      *             'ISO 1300,14PLAN APRES',ISO(IPLOT),
C      *             'GOODELT',GOODELT(IPLOT),'NSP',NSP(IPLOT)
C               IF ((IPLOT==18851).AND.(IPID==7)) WRITE(*,*)
C      *             'ISO 729,14 PLAN APRES',ISO(IPLOT),
C      *             'GOODELT',GOODELT(IPLOT),'NSP',NSP(IPLOT)
            ENDIF
C         IF (IPID==7)  THEN
C           DO IPLAN=1,NPLAN
C             IF (IPLOT==(729+(IPLAN-1)*NPOIN2)) THEN
C             WRITE(160+IFF,*)'PT(729,',IPLAN,'), SHP1',
C      *        SHP1(IPLOT),'SHP2',SHP2(IPLOT),'SHP3',SHP3(IPLOT),
C      *        'SHZ',SHT(IPLOT),'SHF',SHF(IPLOT),
C      *        'ISP',ISP,'DX',DX(IPLOT),'DY',DY(IPLOT),
C      *        'DZ',DW(IPLOT),'DF',DF(IPLOT)
C             ENDIF
C           ENDDO
C         ENDIF
C         IF (IPID==6)  THEN
C           DO IPLAN=1,NPLAN
C             IF (IPLOT==(1300+(IPLAN-1)*NPOIN2)) THEN
C             WRITE(60+IFF,*)'PT(1300,',IPLAN,'), SHP1',
C      *        SHP1(IPLOT),'SHP2',SHP2(IPLOT),'SHP3',SHP3(IPLOT),
C      *        'SHZ',SHT(IPLOT),'SHF',SHF(IPLOT),
C      *        'ISP',ISP,'DX',DX(IPLOT),'DY',DY(IPLOT),
C      *        'DZ',DW(IPLOT),'DF',DF(IPLOT)
     ...
     &            .AND.(ISP==1)) THEN
                ...
                ELSEIF (ISOH.EQ.96) THEN
                   IF (IFA==1) THEN
                      ...
                      ENDIF
                   ENDIF
C     IF ((GOODELT(IPLOT)==1101).AND.(IFABOR(IEL,IFA)==-2)
C     *                                           .AND.ISP==1) THEN
C     IFA=ISUI(IFA)
C     ENDIF
...
C     IF ((IPLOT==5736).AND.(IPID==1)) THEN
C     WRITE(*,*)
C      *            'ISO 1434,3PLAN PHASE 2',ISO(IPLOT),'GOODELT',
C     *        GOODELT(IPLOT),'DX',DX(IPLOT),'DY',DY(IPLOT),'SHP1',
C     *        SHP1(IPLOT),'SHP2',SHP2(IPLOT),'SHP3',SHP3(IPLOT),
C     *        'IEL',IEL,'ISP',ISP
C     ENDIF
C     IF ((IPLOT==5597).AND.(IPID==0)) THEN
C     WRITE(*,*)
C     *           'ISO 1361,3 PLAN PHASE 2',ISO(IPLOT),'GOODELT',
C     *        GOODELT(IPLOT),'DX',DX(IPLOT),'DY',DY(IPLOT),'SHP1',
C      *        SHP1(IPLOT),'SHP2',SHP2(IPLOT),'SHP3',SHP3(IPLOT),
C     *        'IEL',IEL,'ISP',ISP
C     ENDIF
