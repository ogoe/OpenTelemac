...
!                    ******************
!
     & ( F , B , SHP , SHZ , SHF , ELT , ETA , FRE ,
     &   IKLE2 , ETAP1, NPOIN2 , NPOIN3, NELEM2 , NPLAN , NF , TRA01)
!
!***********************************************************************
...
!+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
!
      INTEGER NELEM2,NPOIN2,NPOIN3,NPLAN,NF      
!
...
!
      ...
      TYPE(BIEF_OBJ), INTENT(IN) :: SHP,SHZ,SHF
!
!+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
!
      INTEGER ELT2,ETA2,FRE2,NNSEND,NNRECV,IP2,IP,IFF,IPLAN,IFR,IP3
      INTEGER ETAG,ETAGP1,IPOIN1,IPOIN2,IPOIN3
      DOUBLE PRECISION UMSHZ,UMSHF,SH1,SH2,SH3,SHZZ,SHFF
!
!-----------------------------------------------------------------------
!
!
...
!
      DO 10 IFF=1,NF
        DO 20 IPLAN = 1 , NPLAN
          DO 30 IP = 1 , NPOIN2
         IP3=IP+(IPLAN-1)*NPOIN2
         SH1 =SHP%ADR(IFF)%P%R(3*(IP3-1)+1)
         SH2 =SHP%ADR(IFF)%P%R(3*(IP3-1)+2)
         SH3 =SHP%ADR(IFF)%P%R(3*(IP3-1)+3)
         SHZZ=SHZ%ADR(IFF)%P%R(IP3)
         SHFF=SHF%ADR(IFF)%P%R(IP3)
     ...
     &      ((TRA01(IPOIN1,ETAG,IFR) * SH1
     &      + TRA01(IPOIN2,ETAG,IFR) * SH2
     &      + TRA01(IPOIN3,ETAG,IFR) * SH3) * UMSHZ
     &     +( TRA01(IPOIN1,ETAGP1,IFR) * SH1
     &      + TRA01(IPOIN2,ETAGP1,IFR) * SH2
     &      + TRA01(IPOIN3,ETAGP1,IFR) * SH3)
     &        * SHZZ ) + SHFF *
     &      ((TRA01(IPOIN1,ETAG,IFR+1) * SH1
     &      + TRA01(IPOIN2,ETAG,IFR+1) * SH2
     &      + TRA01(IPOIN3,ETAG,IFR+1) * SH3) * UMSHZ
     &     +( TRA01(IPOIN1,ETAGP1,IFR+1) * SH1
     &      + TRA01(IPOIN2,ETAGP1,IFR+1) * SH2
     &      + TRA01(IPOIN3,ETAGP1,IFR+1) * SH3)
     &            * SHZZ))  /B(IP,IFF)
