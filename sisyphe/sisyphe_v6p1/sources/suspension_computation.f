...
!     IN TELEMAC-2D OR 3D, WITH SOLSYS=2, DM1 AND ZCONV ARE USED BUT
!     ARE NOT YET BUILT WHEN SISYPHE IS CALLED AT THE FIRST TIME STEP
!     WHAT'S MORE, UCONV_TEL AND VCONV_TEL ARE THEN DIFFERENT FROM
!     U2D AND V2D
!
      SOLSYS_SIS=1
      IF(LT.NE.1.AND.CODE(1:7).EQ.'TELEMAC') SOLSYS_SIS=SOLSYS
!     SEE FURTHER DOWN A MODIFICATION TO SOLSYS_SIS : IF(CORR_CONV)...
!
!
!
...
!
!     COMPUTES THE REFERENCE ELEVATION  -->  ZREF
!
...
!       fin modif NH
!
!     COMPUTES THE ADVECTION VELOCITIES
!     TAKING INTO ACCOUNT THE VERTICAL PROFILE OF CONCENTRATIONS AND VELOCITIES
!
!     OPTVF : TENS                  0 : NORMAL
...
!
!        TEMPORARILY USES U2D AND V2D HERE
!        EVEN THOUGH SOLSYS_TEL=2 BECAUSE SHOULD ALSO CORRECT DM1
         SOLSYS_SIS=1
         CALL CPSTVC(U2D,T12)
...
!        THAT DOES NOT SATISFY CONTINUITY + CHI-TUAN CONSTANT
         OPTVF=11
!
      ELSE
!
...
!
        IF(SOLSYS_SIS.EQ.1) THEN
          ...
          VCONV%R=>V2D%R
        ELSE
!         HERE UCONV_TEL WAS PASSED ON
          UCONV%R=>UCONV_TEL%R
          VCONV%R=>VCONV_TEL%R
        ENDIF
!        ADVECTION FORM THAT REQUIRES AN ADVECTION FIELD
!        THAT SATISFIES CONTINUITY + CHI-TUAN CONSTANT
         OPTVF=1
!
      ENDIF
!
...
!
     ...
     &               MESH%SURDET)
      ...
      ENDIF
!
!     FLUX COMPUTATION AT TIME N (CALLED FOR THE FIRST CLASS
!                                     WHICH MUST BE THE SAND)
!
! DEPOSITION IS TREATED THE SAME WAY FOR MIXES OR SEDIMENTS ALONE
!!---> T2: RATION BETWEEN BOTTOM CONCENTRATION AND AVERAGE
!          CONCENTRATION
!
...
!         CSF=CSF_VASE ! DEFINED IN MIXED SEDIMENT
     ...
     &         TOCE_MIXTE%R,MS_SABLE%R,MS_VASE%R)
      ...
      ENDIF
!
! ....END OF MODIFICATION FOR MIXED SEDIMENTS
!
!     TREATS SMALL DEPTHS
!
      IF(OPTBAN.EQ.1) THEN
...
!     OVERWRITTEN
!
!     T2 = RATIO BETWEEN BOTTOM CONC. AND AVERAGE CONC. MUST BE KEPT UNTIL THIS STAGE
!
...
!
        DO K = 1, NPTFR
          IF(CLT%I(K).EQ.KENT) THEN
            I = MESH%NBOR%I(K)
            IF(.NOT.SEDCO) THEN
!V SEE MODIFICATIONS IN SUSPENSION_EROSION
!V               CBOR%R(K) = CSTAEQ%R(I)/T2%R(I)*AVA(I)
...
!         HN_TEL IS NOT CLIPPED
        ...
        ENDIF
      ELSE
!       IN THIS CASE H AND HN ARE CONFUSED
        HOLD=>HN
      ENDIF
!
...
!               OPTSOU
     ...
     &     S   ,  .FALSE.  )
!       FLULIM   YAFLULIM    : HERE YAFLULIM SET TO FALSE AND S
!                              VOID STRUCTURE. WHEN YAFLULIM AND FLULIM
!                              CAN BE OBTAINED FROM TELEMAC2D
!                              IT WILL SPEED-UP TIDAL FLAT VERSIONS
