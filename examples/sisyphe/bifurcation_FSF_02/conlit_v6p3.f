...
!+        01/06/2013
!+        V6P3
!+   Correction for multiples boundaries
!+   Thanks to Dougal Clunie from the Forum
!+   to point us this error and submitt
!+   a correction
!
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!
      ...
      DOUBLE PRECISION RATIO, TEMP(NPTFR,NSICLA)
!
...
!    
            DO K=1,NPTFR
               IF(NUMLIQ%I(K).EQ.IFRLIQ) THEN
                  TEMP(K,1)= QBOR%ADR(1)%P%R(K)
               ENDIF
            ENDDO
!           MULTICLASS CASE (AS FOR EBOR ABOVE)
     ...
     &              LIQBOR%I(K).EQ.KENT) THEN
                    TEMP(K,I)=AVAIL(NBOR(K),1,I)*QBOR%ADR(1)%P%R(K)
                   ENDIF
                ENDDO
              ENDDO 
            ENDIF
      ...
      DO  K=1,NPTFR
         IF(LIQBOR%I(K).EQ.KENT)   THEN
            QBOR%ADR(1)%P%R(K) = TEMP(K,1)
            IF(NSICLA.GT.1) THEN
               DO I=NSICLA,1,-1
                  QBOR%ADR(I)%P%R(K) = TEMP(K,I)
               ENDDO
            ENDIF
         ENDIF
      ENDDO  
