...
!
!       PAROIS SOLIDES
        IF(JB.GE.2.AND.JB.LE.440)THEN
          LIHBOR%I(I) = KLOG
          RP%R(I) = 1.D0
          TETAP%R(I) = 90.D0
          ALFAP%R(I) = 0.D0
        ENDIF
        IF(JB.GE.441.AND.JB.LE.445)THEN
          LIHBOR%I(I) = KSORT
          TETAP%R(I) = 0.D0
        ENDIF
!
...
!
        IF(JB.GE.446.AND.JB.LE.884)THEN
          LIHBOR%I(I) = KLOG
          RP%R(I) = 1.D0
          TETAP%R(I) = 90.D0
          ALFAP%R(I) = 0.D0
...
!
        IF(JB.GE.885.AND.JB.LE.888)THEN
          LIHBOR%I(I)= KINC
          HB%R(I)    = SQRT(2D0)
          TETAB%R(I) = 0.D0
          TETAP%R(I) = 0.D0
...
!
      DO I = 1,NPOIN
        ...
        IF (X(I).GE.30.D0) THEN
          ZF%R(I) = 0D0+4D0/(190D0-30D0)*(X(I)-30D0)
        ENDIF
