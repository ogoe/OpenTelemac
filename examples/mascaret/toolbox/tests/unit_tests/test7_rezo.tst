...
// initialisation
erreur = initStateNameMASCARET(id,toolbox_dir+c+".."+c+"test"+c+"Test_Plan"+c+"Test7"+c+"data"+c+"REZO"+c+"cas"+c+"mascaret0.lig",impression);
assert_checkequal(erreur,0);
...
ZTP = zeros(1001);
[erreur,ZTP(1)] = getDoubleMASCARET(id,"State.Z",41,0,0); // x = 4000 m en section no. 41
assert_checkequal(erreur,0);
...
while (tpsCalcul <= TF)
  ...
  [erreur,ZTP(i)] = getDoubleMASCARET(id,"State.Z",41,0,0); // x = 4000 m en section no. 41
  assert_checkequal(erreur,0);
  i = i + 1;
end
