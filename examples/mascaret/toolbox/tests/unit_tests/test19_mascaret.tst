...
path_xml = "file://"+toolbox_dir+c+".."+c+"test"+c+"Test_Plan"+c+"Test19"+c+"data"+c+"MASCARET"+c+"xml"+c+"mascaret0.xcas";
TabNomFichier = [
                 strsubst(path_xml,'\','/'), ..
                 toolbox_dir+c+".."+c+"test"+c+"Test_Plan"+c+"Test19"+c+"data"+c+"MASCARET"+c+"xml"+c+"mascaret0.geo", ..
                 toolbox_dir+c+".."+c+"test"+c+"Test_Plan"+c+"Test19"+c+"data"+c+"MASCARET"+c+"xml"+c+"mascaret0_0.loi", ..
                 toolbox_dir+c+".."+c+"test"+c+"Test_Plan"+c+"Test19"+c+"data"+c+"MASCARET"+c+"xml"+c+"mascaret0_1.loi", ..
                 toolbox_dir+c+".."+c+"test"+c+"Test_Plan"+c+"Test19"+c+"data"+c+"MASCARET"+c+"xml"+c+"mascaret0_2.loi", ..
...
// initialisation
erreur = initStateNameMASCARET(id,toolbox_dir+c+".."+c+"test"+c+"Test_Plan"+c+"Test19"+c+"data"+c+"MASCARET"+c+"xml"+c+"mascaret0.lig",impression);
assert_checkequal(erreur,0);
...
for i = 1:nbSec
    ...
    assert_checkequal(erreur,0);
    [erreur,F(i)] = getDoubleMASCARET(id,"State.Froude",i,0,0);
    assert_checkequal(erreur,0);
    [erreur,ZF(i)] = getDoubleMASCARET(id,"Model.Zbot",i,0,0);
    assert_checkequal(erreur,0);
    H(i) = Z(i) - ZF(i);
end
...
code_retour = assert_checkalmostequal(Q(1:569),12.36*ones(1:569)',%eps,1.5D0);
assert_checktrue(code_retour);
code_retour = assert_checkalmostequal(Q(570:nbSec),13.21*ones(1:316)',%eps,1.D-2);
assert_checktrue(code_retour);

// test de la solution sur la cote / solution anterieure de reference
code_retour = assert_checkalmostequal(Z,ResRef(:,6),1.D-4);
assert_checktrue(code_retour);

// test de la solution sur la hauteur / solution anterieure de reference
code_retour = assert_checkalmostequal(H,ResRef(:,8),2.D-2);
assert_checktrue(code_retour);

// test de la solution sur le nombre de Froude / solution anterieure de reference
code_retour = assert_checkalmostequal(F,ResRef(:,7),4.D-2);
assert_checktrue(code_retour);
