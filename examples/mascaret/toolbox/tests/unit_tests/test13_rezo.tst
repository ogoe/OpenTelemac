...
// initialisation
erreur = initStateNameMASCARET(id,toolbox_dir+c+".."+c+"test"+c+"Test_Plan"+c+"Test13"+c+"data"+c+"REZO"+c+"xml"+c+"mascaret0.lig",impression);
assert_checkequal(erreur,0);
...
[erreur,Q0(1)] = getDoubleMASCARET(id,"State.Q",1,0,0); // section amont
assert_checkequal(erreur,0);
[erreur,Q2000(1)] = getDoubleMASCARET(id,"State.Q",21,0,0); // section intermediaie
assert_checkequal(erreur,0);
[erreur,Q4000(1)] = getDoubleMASCARET(id,"State.Q",41,0,0); // section aval
assert_checkequal(erreur,0);
...
while (tpsCalcul <= TF)
  ...
  [erreur,Q0(i)] = getDoubleMASCARET(id,"State.Q",1,0,0);
  assert_checkequal(erreur,0);
  [erreur,Q2000(i)] = getDoubleMASCARET(id,"State.Q",21,0,0);
  assert_checkequal(erreur,0);
  [erreur,Q4000(i)] = getDoubleMASCARET(id,"State.Q",41,0,0);
  assert_checkequal(erreur,0);
  i = i + 1;
end
