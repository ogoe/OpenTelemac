...
!
      IF(NTRAC.GT.0) THEN
        ...
        DO I=1,NPOIN
          DO ITRAC=1,NTRAC
            X_INI = MESH%X%R(I)-10.D0
            Y_INI = MESH%Y%R(I)-10.D0
            T%ADR(ITRAC)%P%R(I) = (1.D0 - 16.D0*PI**2/L_BOX**2)
     &                          * SIN(2.D0*PI/L_BOX*X_INI)
     &                          * SIN(2.D0*PI/L_BOX*Y_INI)
            TN%ADR(ITRAC)%P%R(I) = T%ADR(ITRAC)%P%R(I)
            TTILD%ADR(ITRAC)%P%R(I) = T%ADR(ITRAC)%P%R(I)
          ENDDO
        ENDDO
      ENDIF
!
