...
!         T%R(IPOIN) = 1.D0 + EXP(-EIKON)
          T%ADR(1)%P%R(IPOIN) = EXP(-EIKON)
        ENDDO
      ENDIF
C
C-----------------------------------------------------------------------
...
C       IF(CLT(K).EQ.KENT) THEN
C         USCALN = U(NBOR(K))*XNEBOR(K) + V(NBOR(K))*YNEBOR(K)
!         OUTGOING VELOCITY, FREE TRACER
C         IF(USCALN.GT.0.D0) CLT(K) = KSORT
C       ELSEIF(CLT(K).EQ.KSORT) THEN
C         USCALN = U(NBOR(K))*XNEBOR(K) + V(NBOR(K))*YNEBOR(K)
!
!         INCOMING VELOCITY, TRACER IMPOSED AT THE LAST VALUE
C         IF(USCALN.LT.0.D0) THEN
C           TBOR%R(K)=TN%R(NBOR(K))
C           CLT(K) = KENT
C         ENDIF
C       ENDIF
      ENDDO
