...
C                             2:HARDCODED OPTION FOR ALGORITHM
C                               INDEPENDENT OF SEGMENT NUMBERING.
C       IF EXITS AT THIS POINT, THE DIRICHLET ARE NOT DONE, ALSO WORKS
C       CAN THEN CHECK THE MASS CONSERVATION EXACTLY
        IF(.NOT.DIFT) RETURN
        CALL MATVEC( 'X=AY    ',SM,AM1,F,C,MESH)
...
C
C     DIRICHLET BOUNDARY CONDITIONS ARE TREATED IN A WEAK FORM BY
C     FINITE VOLUME SCHEMES, TO ENSURE THE CORRECT FLUX
C     (THEN CALL DIRICH HERE WILL SPOIL MASS CONSERVATION)
C
C
!     IF(.NOT.FV_SCHEME) THEN
