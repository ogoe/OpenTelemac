...
!><br>            CASE OF QUASIBUBBLE-LINEAR ELEMENT.
...
C
      ...
      ENDDO
C
C  ASSEMBLES PART P1 (BETWEEN 1 AND 2*NSEG11)
C
      DO IELEM = 1,NELEM
C         TERM 12
...
C         TERM 13
     ...
     &  = XM(ELTSEG3(IELEM)+NSEG11*(2-ORISEG3(IELEM))) + XMT(IELEM,02)
      ENDDO
C
C  ASSEMBLES THE QUASIBUBBLE PART
C  BETWEEN XM(2*NSEG11+1) AND XM(NSEG11+NSEG12)
C  SEE IN STOSEG HOW ELTSEG4,5 AND 6 ARE BUILT
C
      DO IELEM = 1,NELEM
C       TERM 41
